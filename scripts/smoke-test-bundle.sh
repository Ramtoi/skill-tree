...
# hub.py imports these hooks-surface modules at runtime (harness_probe/
# tool_catalog/hooks_model/hook_adapters) — missing any one crashes `hub sync`
# with ModuleNotFoundError in the installed app.
for mod in harness_probe tool_catalog hooks_model hook_adapters; do
  [[ -f "$RES/hub/$mod.py" ]] || fail "bundled $mod.py missing: $RES/hub/$mod.py"
done
