...
def test_cli_source_add_already_imported(tmp_data_home, monkeypatch, tmp_path):
    """Re-adding the same source-id with the same skill yields IMPORTED in classification.

    We test classification semantics via dry-run on a registry that already has
    an external entry for the source-id.
    """
    code_root = _seed_code_home(tmp_data_home, monkeypatch)
    _seed_registry(
        tmp_data_home,
