...
@pytest.fixture(autouse=True)
def _isolate_harness_agent_state(request, tmp_path_factory, monkeypatch):
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers",
        "real_harness_paths: opt out of the harness agent/global-doc isolation guard "
        "(only for tests asserting the harness registry's declared literals)")
