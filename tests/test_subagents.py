...
def claude_home(tmp_path, monkeypatch):
    ...
    And the DATA home, which is not the same thing: every mutating verb here
    backs the prior file up under `data_home()/_hub-backups/subagents/` first,
    so isolating only the Claude home left `save`/`rename`/`delete`/`disable`
    writing real backup files into the developer's own `~/.skill-hub` on every
    run. `hub._DATA_HOME_CACHE` has to be cleared too — it is a module-level
    memo, so a test that ran earlier in the same process would otherwise pin
    the resolution regardless of the env.
    """
    ...
    monkeypatch.setenv("SKILL_HUB_HOME", str(data_home))
    monkeypatch.delenv("SKILL_HUB_DIR", raising=False)
    monkeypatch.setattr(hub, "_DATA_HOME_CACHE", None, raising=False)
    return home
...
def test_mutations_back_up_into_the_isolated_data_home_not_the_real_one(
    claude_home, tmp_path
):
    """Isolating ~/.claude is not enough — the BACKUPS go to the data home.

    `_backup_file` writes under `hub.data_home()/_hub-backups/subagents/`, which
    for a long time resolved to the developer's real `~/.skill-hub` because this
    suite's fixture never overrode it (nor cleared the module-level resolution
    memo). Every `save`/`rename`/`delete`/`disable` test then dropped real files
    outside its tmp dir. Fail loudly if that isolation regresses.
    """
    import hub

    resolved = hub.data_home()
    assert resolved == tmp_path / "datahome", f"data home escaped the fixture: {resolved}"

    # A prior settings.json is what makes the mutation take a backup at all.
    (claude_home / "settings.json").write_text(json.dumps({"model": "opus"}))
    subagents.set_disabled("foo", True, "user")

    backups = list((resolved / "_hub-backups" / "subagents").glob("*.bak"))
    assert backups, "the mutation should have backed something up (inside tmp)"
    for path in backups:
        assert str(path).startswith(str(tmp_path)), f"backup escaped tmp: {path}"
