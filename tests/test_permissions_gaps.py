...
def test_codex_no_longer_drops_permission_block_hooks():
    """Retirement (hooks-surface D3/D6): codex IS hook-capable and hooks moved to
    the hook library, so the codex permissions adapter no longer skips a stray
    permission-block hook nor raises the retired DROPPED_HOOK "Codex has no hooks"
    finding.
    """
    ...
    result = pa.CodexPermissionAdapter().translate(perms, GlobalScope(), "codex")
    assert not any(s.feature == "hooks" for s in result.skipped)
    assert not any(f.code == "DROPPED_HOOK" for f in result.risks)
