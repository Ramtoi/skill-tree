...
# NOTE: the codex-workers sign-on-push counterpart of the test above lives with
# its connector, in the private plugin repo (`tests/test_codex_signing_integrity.py`).
# It imports `MockSshKeygen` from THIS module by path, so keep that class's name
# and constructor stable.
