...
@pytest.mark.real_harness_paths
...
def test_emit_schema_carries_hooks_mechanism():
    """emit_schema gains a static per-harness hooks.mechanism block (D4).

    This is the build-time shape, distinct from harness_probe.py's runtime
    verdicts. Every entry carries it; the mechanism is pinned per harness.
    """
    import harnesses

    schema = {e["id"]: e for e in harnesses.emit_schema()}
    for entry in schema.values():
        assert "hooks" in entry, f"{entry['id']} missing hooks block"
        assert "mechanism" in entry["hooks"]
        assert entry["hooks"]["mechanism"] in {"command", "plugin", "none"}
    assert schema["claude-code"]["hooks"]["mechanism"] == "command"
    assert schema["codex"]["hooks"]["mechanism"] == "command"
    assert schema["pi"]["hooks"]["mechanism"] == "none"
    assert schema["opencode"]["hooks"]["mechanism"] == "plugin"
