...
def test_non_bash_and_unbounded_are_skipped(tmp_data_home, tmp_path):
    adapter = pa.OpenCodePermissionAdapter()
    scope = _proj_scope(tmp_path)
    # Permission-block hooks are retired (hooks-surface D6) — the hook library +
    # hooks sync stream own hooks now, so the permissions adapter no longer emits
    # a hook skip/risk. A stray perms.hooks entry is silently ignored here.
    perms = NormalizedPermissions(
    ...
    assert PermissionFeature.HOOKS.value not in features
