...
def test_codex_adapter_skips_gracefully_without_tomlkit(tmp_path, monkeypatch):
    """A Python env without tomlkit (e.g. a CI job that never pip-installs it)
    must degrade every Codex MCP write to a clean skip, not crash the whole
    sync with an uncaught ImportError."""
    import mcp_adapters
    from mcp_adapters import CodexMcpAdapter, McpServerSpec

    monkeypatch.setattr(mcp_adapters, "_tomlkit_missing", lambda: True)
    adapter = CodexMcpAdapter()
    proj = tmp_path / "proj"
    proj.mkdir()

    assert adapter.write(proj, [McpServerSpec(name="x", command="python3")]) is False
    assert not (proj / ".codex" / "config.toml").exists()
    assert adapter.remove(proj, {"x"}) is False

    result = adapter.write_global(tmp_path / "global.toml", [], prior_managed=None)
