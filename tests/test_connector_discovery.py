...
per-plugin error isolation, non-shadowing, lazy no-import-side-effect, metadata
defaults) and the `hub remote connectors` catalog command.

Discovery has exactly THREE sources — builtin, entry-point, drop-in. Location is
the shipping boundary: a plugin that is not in `code_home()` cannot enter a build
artifact, so private plugins are drop-ins under `data_home()/connectors/` rather
than a fourth, in-tree source that every tree consumer must remember to exclude.
