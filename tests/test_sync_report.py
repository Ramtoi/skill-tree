...
def test_report_written_with_full_shape(sync_env, capsys):
    ...
    assert set(g.keys()) == {
        ...
        # `backup` is additive (design v2 §9): the frontend's SyncReportGlobal
        # interface is already a subset of what Python emits, so extra keys are
        # tolerated and `schema_version` stays at 1.
        "backup",
    }
