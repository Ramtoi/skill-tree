...
def _run_add(
    ...
    *extra_args: str,
    ...
    env["GIT_TERMINAL_PROMPT"] = "0"
    return subprocess.run(
        [sys.executable, str(REPO_ROOT / "hub.py"), "source", "add", "git", url, *extra_args],
        env=env,
        capture_output=True,
        text=True,
        cwd=str(REPO_ROOT),
...
def _seed_existing_source(tmp_data_home: Path, source_id: str) -> None:
    registry = {
        "version": "1",
        "skills": {},
        "sources": {
...
def test_dry_run_rejects_existing_source_id_without_cloning(
    tmp_data_home, monkeypatch, tmp_path
):
    """Preview (`--dry-run`) must fail up front on a taken id — not clone then
    pass, only to fail at apply. Uses a bogus URL so any clone attempt would
    error differently; the fast-fail happens before we touch git."""
    code_root = _seed_code_home(tmp_data_home, monkeypatch)
    _seed_existing_source(tmp_data_home, "org-skills")
    res = _run_add(
        tmp_data_home,
        code_root,
        "file:///nonexistent/should-not-be-cloned.git",
        "--id",
        "org-skills",
        "--dry-run",
        "--json",
...
def test_dry_run_rejects_reserved_builtin_id(tmp_data_home, monkeypatch, tmp_path):
    code_root = _seed_code_home(tmp_data_home, monkeypatch)
    res = _run_add(
        tmp_data_home,
        code_root,
    ...
    assert res.returncode != 0
    assert "reserved" in (res.stdout + res.stderr).lower()
