...
- `docs/remote-connectors.md` — pushing hub-managed artifacts to remotes via pluggable connectors: the `remotes:` block (references-only/no-secrets), the `RemoteConnector` plan-then-apply ABC + `REMOTE_CONNECTORS` registry, the publishable Hermes connector (`~/.hermes/skill-hub/` push, merge-preserving config, SOUL/MEMORY/USER round-trip, write-confinement), the `_run_remote_dispatch` sync pass + 3-way drift/conflict model, import (`origin: remote:<id>`), TOFU host-key pinning + keychain secrets, `hub remote …` CLI, doctor, authoring a connector. Topics routed here: *remote*, *connector*, *hermes remote*, *remote sync*, *remote drift*, *equip remote*, *push skill to box*.
