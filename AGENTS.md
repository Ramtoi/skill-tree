...
- `docs/HOOKS.md` — hooks as a separate top-level library (`hooks:` + `hooks_global`/`projects.<n>.hooks` attach lists + `hook_settings`), `builtin`/`user` provenance (built-ins ship in `code_home()/hooks/`, not the registry; shadow-by-registry-name), `harness_probe.py`'s 4-verdict capability model (supported/feature_off/unsupported/not_installed) + `state/harness-capabilities.json` cache + per-event gating (claude-code 14-event catalog / codex exact 10) + tool→matcher translation (`Edit|Write|MultiEdit`→`apply_patch` on codex), the dedicated hooks sync stream (`--skip-hooks`, after permissions, disjoint `kind="hooks"` sidecar, scope→file map: claude project → `settings.local.json`, codex v1 global-only), Codex read-only trust posture (`[hooks.state]` never written), the `lsp-report` built-in (PostToolUse ruff/tsc/cargo/gopls, advisory vs blocking, single-flight, silent missing-checker), doctor findings (`HOOK_RUNS_SUDO`/`HOOK_BROKEN_SCRIPT`/`LSP_CHECKER_MISSING`), `hub hook …` CLI, deprecated `hub permissions hooks add/remove` aliases. Topics routed here: *hook*, *hooks*, *lsp-report*, *PostToolUse*, *hook trust*, *hook capability*, *feature-off*, *attach hook*, *disable hook*.
...
- `docs/remote-connectors.md` — pushing hub-managed artifacts to remotes via pluggable connectors: the `remotes:` block (references-only/no-secrets), the `RemoteConnector` plan-then-apply ABC + `REMOTE_CONNECTORS` registry (lazy fail-safe discovery: builtin → `skill_hub.connectors` entry points → drop-in `data_home()/connectors/`; **location is the shipping boundary** — in `code_home()` ⇒ public + `publishable = True` (pytest-enforced), private/local plugins are data-home drop-ins from their own repo; `hub remote connectors --json` catalog; connector metadata `label`/`description`/`transport_kind` drives registry-derived wizard cards + transport-aware onboarding, `setup_key_transport` hook for custom key flows, `setup_helper_plan` hook behind the connector-agnostic `hub remote setup-helper <id>`; `scripts/guard-bundle-privacy.sh` + `scripts/private-identifiers.txt` guard the built bundle), the publishable Hermes connector (`~/.hermes/skill-hub/` push, merge-preserving config, SOUL/MEMORY/USER round-trip, write-confinement), the `_run_remote_dispatch` sync pass + 3-way drift/conflict model, import (`origin: remote:<id>`), TOFU host-key pinning + keychain secrets, `hub remote …` CLI, doctor, authoring a connector. Topics routed here: *remote*, *connector*, *hermes remote*, *remote sync*, *remote drift*, *equip remote*, *push skill to box*.
...
- `docs/BACKUP.md` — full backup & restore: portable git-snapshot repo (`~/.skill-hub-backup/`, pushed to a private GitHub repo) assembled by `backup.py` from the single `MANIFEST` table + out-of-home gathering (sub-agents, global agent docs); field-scoped path transform (`{HOME}`/`{DATA_HOME}`/`{CODE_HOME}` tokens, `mcp.env` redacted, `hub_path`/`bootstrap`/`signing`/`backup` dropped), three fail-closed secret gates (`--allow-secret <sha256>`), auth ladder (ssh → gh create-only → fine-grained PAT Contents:RW in the OS keychain, stdin-only), sync-tail auto-backup (fail-open-not-silent, fingerprint dirty gate, fetch+reset non-FF-impossible, `--skip-backup`, doctor `BACKUP_STALE`/`BACKUP_AUTH_EXPIRED`), `restore.py`: dry-run default + `--apply`, replace/merge modes, SSHSIG multi-signer TOFU pins (`state/backup-signers.json`), executable-state consent (hooks + permissions + trust + `code_dirs`), three-way sub-agent/global-doc collisions (`.from-backup` siblings), project quarantine (`path_unresolved`, cleared by `edit-path`), `pending_reconcile` push hold (`hub backup now --acknowledge-restore`), `hub source restore`, `hub bootstrap --restore-from`, app Backup screen + wizard restore-first flow (`g ⇧b`). Topics routed here: *backup*, *restore*, *snapshot*, *sync backend*, *machine migration*, *move to new machine*, *backup repo*, *pending reconcile*, *quarantined project*, *unverified snapshot*, *allow-secret*.
...
- `app/` — Skill Tree native desktop app (Tauri 2 + React 19 + TypeScript + Vite)
  ...
  - `app/src-tauri/tauri.conf.json` — app config + `bundle.resources` that ships `hub.py` + its sibling `.py` modules + `requirements.txt` into `Contents/Resources/hub/` (no starter `skills/`/`mcp-servers/` are bundled; the Starter Pack source ships empty and is populated later as an external source), **plus a bundled CPython runtime** into `Contents/Resources/python/` so the app needs no system `python3`. `beforeBuildCommand` runs `vendor-deps.sh` (pure-Python deps) then `fetch-python-runtime.sh` (pinned, checksum-verified [python-build-standalone](https://github.com/astral-sh/python-build-standalone) CPython → gitignored `python-runtime/`). `detect_python()` in `commands/hub.rs` resolves `SKILL_TREE_PYTHON` → bundled → system PATH. `scripts/smoke-test-bundle.sh` (npm `test:bundle`) validates the built `.app` hermetically and gates both install paths (`hub app build --install`, `rebuild-skill-tree-app.sh`). Bump the runtime via the pins in `fetch-python-runtime.sh` + `EXPECT_PY_VERSION` in the smoke test. Caveat: hub-written MCP entries still hardcode `command: "python3"`. See `docs/distribution.md` §Bundled Python runtime.
...
(anything in `code_home()/connectors/` is public by definition and MUST set `publishable = True`);
the maintainer's **custom private connectors live in a separate private plugin repo**
(`publishable = False`) and install as **drop-ins** under `data_home()/connectors/<name>`, so their
bytes are structurally unable to reach `git archive` or `bundle.resources`. `hub sync` runs `_run_remote_dispatch` as a
...
<!-- skill-tree:snippet id=verify-with-tywin v=10 sha=a11eb3d070fb -->
## Code review gate (tywin)

Before EVERY commit, run:

    tywin check --working

- **Exit 0 (PASS):** proceed with the commit.
- **Exit 1 (FINDINGS):** read the JSON findings. Fix all BLOCKER and CRITICAL findings and any
  MAJOR findings that are cheap to fix relative to the diff. Re-run `tywin check --working` until it passes, then commit.
- **Exit 2 (BLOCKED):** the review could not run (compile/test failure, empty diff, provider unreachable). Fix the stated cause first — do not commit.

Pass `--note "<claim>"` to point the reviewer at context it can't see in the diff (e.g. where validation already happens); it verifies claims against the code. Never bypass or disable the gate.
