...
/// Caches ONLY a successful interpreter resolution. A failed detection is never
/// stored, so a later call re-probes — this is what makes "Recheck runtime" able
/// to pick up a Python the user installed *after* launch. (A `OnceLock` here was
/// a bug: it cached the initial `None` for the whole process, so recheck and
/// post-install both appeared to do nothing until a full app restart.)
static PYTHON_PATH: Mutex<Option<String>> = Mutex::new(None);
...
pub fn resolved_python() -> Option<String> {
    let mut guard = PYTHON_PATH.lock().unwrap();
