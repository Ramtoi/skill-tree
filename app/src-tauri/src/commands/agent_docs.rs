...
/// Count of markdown-eligible files anywhere under `dir` (bounded by depth,
/// skipping the same directories `discover_nested` skips). Used only to size
/// each top-level branch's fair share of `MAX_FILES` — see `discover_nested`.
fn count_markdown_files(
    ...
    for entry in entries.flatten() {
        ...
        let new_rel = format!("{rel_prefix}/{name_str}");
...
/// Level-by-level (breadth-first), round-robin walk of a single subtree,
/// bounded by `budget` files and `MAX_DEPTH`. `start_depth` is the depth of
/// `start` itself (1 for an immediate child of the project root). Shared by
/// `discover_nested` to collect each top-level branch up to its fair share.
fn collect_branch(
    start: &Path,
    start_rel: &str,
    start_depth: usize,
    ...
    let mut current_level: Vec<(PathBuf, String)> = vec![(start.to_path_buf(), start_rel.to_string())];
    let mut depth = start_depth;

    'levels: while !current_level.is_empty() && depth <= MAX_DEPTH {
        let mut next_level: Vec<(PathBuf, String)> = Vec::new();
        // One file queue per directory at this level, drained round-robin
        // below so no single directory can starve its siblings.
        let mut file_queues: Vec<VecDeque<String>> = Vec::new();

        for (dir, rel_prefix) in &current_level {
            let entries = match fs::read_dir(dir) {
                Ok(e) => e,
                Err(_) => continue,
            };
            // Sort siblings so the walk (and what survives truncation) is
            // deterministic across runs, not dependent on filesystem
            // enumeration order.
            let mut children: Vec<(std::ffi::OsString, fs::FileType)> = entries
                .flatten()
                .filter_map(|entry| entry.file_type().ok().map(|ft| (entry.file_name(), ft)))
                .collect();
            children.sort_by(|a, b| a.0.cmp(&b.0));

            let mut files_here: VecDeque<String> = VecDeque::new();
            for (name, file_type) in children {
                ...
                if file_type.is_dir() {
                    ...
                    if depth < MAX_DEPTH {
                        next_level.push((dir.join(&name), new_rel));
                    }
                } else if (file_type.is_file() || file_type.is_symlink())
                    && (ALLOWED_BASENAMES.contains(&name_str.as_str())
                        || (include_all_markdown && name_str.ends_with(".md")))
                {
                    ...
                    files_here.push_back(new_rel);
                }
            }
            if !files_here.is_empty() {
                file_queues.push(files_here);
            }
        }

        while file_queues.iter().any(|q| !q.is_empty()) {
            for q in file_queues.iter_mut() {
                let Some(rel) = q.pop_front() else { continue };
                if out.len() >= budget {
                    break 'levels;
                }
                out.push(rel);
...
/// truncation flag.
///
/// Root-level files are always included in full (there are only ever a
/// handful). Each root-level DIRECTORY then gets a fair share of the
/// remaining `MAX_FILES` budget via max-min water-filling: sort branches by
/// their own total file count ascending, and hand each one `min(its own
/// count, remaining_budget / remaining_branches)`. A single level-by-level,
/// round-robin walk (breadth-first, sorted siblings) is not enough on its
/// own — it fixes fairness AMONG SIBLINGS AT THE SAME DEPTH, but a
/// wide-and-shallow branch (e.g. a spec-heavy `openspec/` with hundreds of
/// files a few levels down) still exhausts the WHOLE global budget before a
/// narrow-and-deep branch (e.g. an Android `app/` module whose docs live six
/// package levels down) ever gets a single file, even though both are
/// siblings at the top. Sizing each branch's budget from its OWN total count
/// up front — instead of letting global depth-order decide — guarantees
/// every top-level directory is represented.
fn discover_nested(
    project_root: &Path,
    already: &HashSet<String>,
    include_all_markdown: bool,
    ...
    let mut out: Vec<String> = Vec::new();
    let mut branch_dirs: Vec<(PathBuf, String)> = Vec::new();
    for (name, file_type) in children {
        let name_str = name.to_string_lossy().into_owned();
        if file_type.is_dir() {
            if ignored_dir_names.contains(name_str.as_str()) {
            ...
            if IGNORED_NESTED_PATHS.iter().any(|p| *p == name_str) {
                continue;
            }
            branch_dirs.push((project_root.join(&name), name_str));
        } else if (file_type.is_file() || file_type.is_symlink())
            && (ALLOWED_BASENAMES.contains(&name_str.as_str())
                || (include_all_markdown && name_str.ends_with(".md")))
        {
            if already.contains(&name_str) {
                continue;
            }
            out.push(name_str);
        }
    ...
    let counts: Vec<usize> = branch_dirs
        .iter()
        .map(|(path, rel)| {
            count_markdown_files(path, 1, rel, already, include_all_markdown, &ignored_dir_names)
        })
        .collect();

    let mut order: Vec<usize> = (0..branch_dirs.len()).collect();
    order.sort_by_key(|&i| counts[i]);
    let mut shares = vec![0usize; branch_dirs.len()];
    let mut remaining_budget = MAX_FILES.saturating_sub(out.len());
    let mut remaining_n = order.len();
    for &i in &order {
        let fair = remaining_budget / remaining_n;
        let share = counts[i].min(fair);
        shares[i] = share;
    ...
    for (idx, (path, rel)) in branch_dirs.iter().enumerate() {
        if counts[idx] > shares[idx] {
            truncated = true;
        }
        out.extend(collect_branch(
            path,
            rel,
            1,
            shares[idx],
...
mod tests {
    ...
    #[test]
    fn all_markdown_mode_truncates_at_max_files() {
        let td = setup_project();
        ...
        let markdown_listing =
            list_agent_docs_impl_with_options(project_str(&td), true).expect("list markdown");
        assert!(
            markdown_listing.truncated,
            "all-markdown listing over the cap must set truncated"
        );
        assert!(
            markdown_listing.all_rels.len() <= MAX_FILES,
            "listing must not exceed MAX_FILES, got {}",
            markdown_listing.all_rels.len()
        );
    }

    #[test]
    fn all_markdown_mode_does_not_starve_sibling_dirs_behind_one_oversized_subtree() {
        // Regression test: a single huge, flat directory (e.g. a spec-heavy
        // `openspec/` with hundreds of files) used to fully exhaust MAX_FILES
        // before a sibling directory ever contributed a single file — true
        // even under plain breadth-first-by-level traversal, since one
        // directory's own file dump can alone exceed the cap. Round-robin
        // draining across same-level directories is required to fix it.
        // Named so the sibling sorts *after* the hog (`zzzz-` > `aaaa-`) —
        // the adversarial case a naive "process directories in order" fix
        // would still fail.
        let td = setup_project();
        let hoggy = td.path().join("aaaa-hoggy");
        fs::create_dir_all(&hoggy).unwrap();
        for i in 0..(MAX_FILES * 2) {
            fs::write(hoggy.join(format!("f{i}.md")), "# x").unwrap();
        }
        let docs = td.path().join("zzzz-docs");
        fs::create_dir_all(&docs).unwrap();
        fs::write(docs.join("guide.md"), "# guide").unwrap();

        let markdown_listing =
            list_agent_docs_impl_with_options(project_str(&td), true).expect("list markdown");
        assert!(markdown_listing.truncated);
        assert!(
            markdown_listing
                .all_rels
                .iter()
                .any(|r| r == "zzzz-docs/guide.md"),
            "sibling directory must not be starved by one oversized subtree: {:?}",
    ...
    fn all_markdown_mode_represents_a_narrow_deep_branch_alongside_a_wide_shallow_one() {
        // Regression test: a plain level-by-level walk fixes fairness AMONG
        // SIBLINGS AT THE SAME DEPTH, but a wide-and-shallow branch (many
        // files a couple of levels down, like a spec-heavy openspec/) still
        // exhausts the whole global budget before a narrow-and-deep sibling
        // (few files many levels down, like an Android app/ module) ever
        // contributes anything — every early "round" is monopolized by the
        // wide branch's own files while the deep one is still just
        // descending empty directories. Each branch must get a budget sized
        // from its OWN total count, not from how deep it happens to be.
        let td = setup_project();
