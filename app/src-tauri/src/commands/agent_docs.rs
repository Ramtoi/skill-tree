...
#[tauri::command]
pub fn list_agent_docs(project_path: String) -> Result<AgentDocsListing, String> {
...
#[tauri::command]
pub fn agent_docs_root_status(project_path: String) -> Result<AgentDocRootStatus, String> {
...
#[tauri::command]
pub fn agent_docs_strategy_get(
...
#[tauri::command]
pub fn agent_docs_strategy_set(
    project_name: Option<String>,
    value: Option<String>,
    clear: Option<bool>,
) -> Result<AgentDocStrategyInfo, String> {
...
#[tauri::command]
pub fn agent_docs_fix_plan(project_path: String) -> Result<Value, String> {
...
mod tests {
    ...
    fn known_paths_always_included() {
        let td = setup_project();
        let listing = list_agent_docs(project_str(&td)).expect("list");
    ...
    fn instruction_sets_group_same_directory_and_extract_title() {
        ...
        let listing = with_policy(multi_policy("symlink"), || {
            list_agent_docs(project_str(&td)).expect("list")
        });
        let set = listing
            .instruction_sets
            .iter()
    ...
    fn rejects_traversal() {
        let td = setup_project();
        let err = read_agent_doc(project_str(&td), "../CLAUDE.md".into())
            .err()
    ...
    fn rejects_absolute() {
        let td = setup_project();
        let err = read_agent_doc(project_str(&td), "/etc/passwd".into())
            .err()
    ...
    fn write_then_read_roundtrip() {
        ...
        let read = read_agent_doc(project_str(&td), "CLAUDE.md".into()).expect("read");
    ...
    fn saving_agents_md_never_touches_existing_real_claude_md() {
        ...
        let read = read_agent_doc(project_str(&td), "AGENTS.md".into()).expect("read");
        let res = with_policy(multi_policy("symlink"), || {
            write_agent_doc(
                project_str(&td),
                "AGENTS.md".into(),
    ...
    fn read_marks_symlinked_claude_as_derived_pointer() {
        ...
        symlink("AGENTS.md", td.path().join("CLAUDE.md")).unwrap();
        let res = read_agent_doc(project_str(&td), "CLAUDE.md".into()).expect("read");
    ...
    fn nested_claude_with_pointer_body_is_not_treated_as_root_derived() {
        ...
        let res = read_agent_doc(project_str(&td), "feature/CLAUDE.md".into()).expect("read");
        assert!(!res.is_derived_pointer);
    }
