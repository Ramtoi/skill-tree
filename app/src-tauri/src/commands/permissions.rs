...
#[tauri::command]
pub async fn permissions_recent_imports(scope: Scope) -> Result<Vec<RecentImport>, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_recent_imports_impl(scope))
        .await
        .map_err(|e| format!("permissions_recent_imports task failed: {e}"))?
}

fn permissions_recent_imports_impl(scope: Scope) -> Result<Vec<RecentImport>, String> {
    let data = data_home()?;
...
#[tauri::command]
pub async fn permissions_show(scope: Scope, personal: Option<bool>) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_show_impl(scope, personal))
        .await
        .map_err(|e| format!("permissions_show task failed: {e}"))?
}

fn permissions_show_impl(scope: Scope, personal: Option<bool>) -> Result<Value, String> {
    let mut args: Vec<String> = vec!["permissions".into(), "show".into()];
    args.extend(scope.flags());
    if personal.unwrap_or(false) {
        args.push("--personal".into());
    }
    args.push("--json".into());
...
pub async fn permissions_set(
    ...
    tauri::async_runtime::spawn_blocking(move || permissions_set_impl(scope, payload, personal))
        .await
        .map_err(|e| format!("permissions_set task failed: {e}"))?
}

fn permissions_set_impl(
    scope: Scope,
    payload: Value,
    personal: Option<bool>,
) -> Result<Value, String> {
...
#[tauri::command]
pub async fn permissions_validate(kind: String, pattern: String) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_validate_impl(kind, pattern))
        .await
        .map_err(|e| format!("permissions_validate task failed: {e}"))?
}

fn permissions_validate_impl(kind: String, pattern: String) -> Result<Value, String> {
    let args = [
        "permissions",
        "validate",
        "--kind",
...
#[tauri::command]
pub async fn permissions_capabilities() -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(permissions_capabilities_impl)
        .await
        .map_err(|e| format!("permissions_capabilities task failed: {e}"))?
}

fn permissions_capabilities_impl() -> Result<Value, String> {
...
#[tauri::command]
pub async fn permissions_migrate_scope(apply: bool) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_migrate_scope_impl(apply))
        .await
        .map_err(|e| format!("permissions_migrate_scope task failed: {e}"))?
}

fn permissions_migrate_scope_impl(apply: bool) -> Result<Value, String> {
...
#[tauri::command]
pub async fn permissions_doctor(scope: Option<Scope>) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_doctor_impl(scope))
        .await
        .map_err(|e| format!("permissions_doctor task failed: {e}"))?
}

fn permissions_doctor_impl(scope: Option<Scope>) -> Result<Value, String> {
...
#[tauri::command]
pub async fn permissions_adopt(
    scope: Scope,
    action: String,
    harness: Option<String>,
) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_adopt_impl(scope, action, harness))
        .await
        .map_err(|e| format!("permissions_adopt task failed: {e}"))?
}

fn permissions_adopt_impl(
    scope: Scope,
    action: String,
    harness: Option<String>,
) -> Result<Value, String> {
...
#[tauri::command]
pub async fn permissions_import_candidates(
    scope: Scope,
    harness: Option<String>,
) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_import_candidates_impl(scope, harness))
        .await
        .map_err(|e| format!("permissions_import_candidates task failed: {e}"))?
}

fn permissions_import_candidates_impl(
    scope: Scope,
    harness: Option<String>,
) -> Result<Value, String> {
...
#[tauri::command]
pub async fn permissions_import_apply(scope: Scope, decisions: Value) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_import_apply_impl(scope, decisions))
        .await
        .map_err(|e| format!("permissions_import_apply task failed: {e}"))?
}

fn permissions_import_apply_impl(scope: Scope, decisions: Value) -> Result<Value, String> {
    let mut args: Vec<String> = vec!["permissions".into(), "import".into()];
    args.extend(scope.flags());
...
#[tauri::command]
pub async fn permissions_reconcile_candidates(
    scope: Scope,
    harness: Option<String>,
) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || {
        permissions_reconcile_candidates_impl(scope, harness)
    })
    .await
    .map_err(|e| format!("permissions_reconcile_candidates task failed: {e}"))?
}

fn permissions_reconcile_candidates_impl(
    scope: Scope,
    harness: Option<String>,
) -> Result<Value, String> {
...
#[tauri::command]
pub async fn permissions_reconcile_apply(scope: Scope, decisions: Value) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_reconcile_apply_impl(scope, decisions))
        .await
        .map_err(|e| format!("permissions_reconcile_apply task failed: {e}"))?
}

fn permissions_reconcile_apply_impl(scope: Scope, decisions: Value) -> Result<Value, String> {
    let mut args: Vec<String> = vec!["permissions".into(), "reconcile".into()];
    args.extend(scope.flags());
    args.push("--apply".into());
    args.push("--decisions-stdin".into());
...
#[tauri::command]
pub async fn permissions_disable(
    target: DisableTarget,
    mode: String,
    apply: bool,
) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || permissions_disable_impl(target, mode, apply))
        .await
        .map_err(|e| format!("permissions_disable task failed: {e}"))?
}

fn permissions_disable_impl(
    target: DisableTarget,
    mode: String,
    apply: bool,
) -> Result<Value, String> {
