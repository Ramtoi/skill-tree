...
/// Pin (or re-pin) a remote's host key via a live keyscan — recovery for an
/// unpinned or rotated key. Maps to `hub remote pin <id> --json` (+ `--yes` to
/// confirm replacing a DIFFERENT existing pin — the possible-MITM case). Returns
/// the JSON payload: `{pinned, changed}` on apply, or `{refused:true, old_pins,
/// new_pin}` when a differing pin is refused (exit 0 in JSON mode so the payload
/// survives). The UI shows a ConfirmDialog with old/new fingerprints, then
/// re-invokes with `yes:true`.
#[tauri::command]
pub async fn remote_pin(id: String, accept: Option<String>, yes: bool) -> Result<Value, String> {
    let mut args: Vec<String> = vec!["remote".into(), "pin".into(), id, "--json".into()];
    if let Some(fpr) = accept {
    ...
    json_off_thread(args).await
}

/// Cheap pre-registration auth probe against a raw ssh-host (the wizard's
/// "Test connection"). Maps to `hub remote probe --ssh-host <host> [--host-key
/// <fpr>] --json`. Returns `{reachable, authenticated, ok, detail, detail_kind}`.
#[tauri::command]
