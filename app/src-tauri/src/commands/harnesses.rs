...
pub struct HarnessMeta {
    ...
    /// Sub-agent capability (present in current schemas; defaulted for old ones).
    #[serde(default)]
    pub agents: AgentsCapability,
    /// User-global instruction-doc path (with `~`), or `None` when the harness
    /// has no such file. `#[serde(default)]` keeps older generated JSON parseable.
    #[serde(default)]
...
fn harness_list_impl() -> Result<Vec<HarnessStatus>, String> {
    ...
    Ok(registry()
        ...
        .map(|h| {
            ...
            let global_doc_abs = h
                .global_doc
                .as_deref()
                .map(|p| expand_tilde(p).to_string_lossy().into_owned());
            let global_doc_exists = global_doc_abs
                .as_deref()
