mod commands;

use commands::hub::resolved_python;

#[cfg_attr(mobile, tauri::mobile_entry_point)]
pub fn run() {
    // Resolve Python at startup so the first runtime_preflight() is instant.
    let _ = resolved_python();

    tauri::Builder::default()
        .plugin(tauri_plugin_opener::init())
        .plugin(tauri_plugin_shell::init())
        .plugin(tauri_plugin_fs::init())
        .plugin(tauri_plugin_dialog::init())
        .plugin(tauri_plugin_updater::Builder::new().build())
        .plugin(tauri_plugin_process::init())
        .invoke_handler(tauri::generate_handler![
            commands::registry::read_registry,
            commands::registry::sync_report,
            commands::registry::read_skill_content,
            commands::registry::read_skill_document,
            commands::registry::write_skill_content,
            commands::registry::save_skill_full,
            commands::registry::local_skill_candidates,
            commands::registry::remote_equip,
            commands::registry::source_add_apply,
            commands::hub::hub_cmd,
            commands::hub::check_python,
            commands::hub::runtime_preflight,
            commands::fs::pick_directory,
            commands::fs::path_exists,
            commands::fs::create_empty_file,
            commands::bootstrap::bootstrap_check,
            commands::bootstrap::bootstrap_run,
            commands::projects::project_add_with_path,
            commands::projects::project_edit_path,
            commands::projects::project_remove_preview,
            commands::projects::project_remove_clean,
            commands::projects::project_scan_candidates,
            commands::harnesses::harness_list,
            commands::harnesses::harness_set_global,
            commands::harnesses::project_set_harnesses,
            commands::global_docs::global_doc_read,
            commands::global_docs::global_doc_write,
            commands::agent_docs::list_agent_docs,
            commands::agent_docs::read_agent_doc,
            commands::agent_docs::write_agent_doc,
            commands::agent_docs::agent_docs_root_status,
            commands::agent_docs::agent_docs_strategy_get,
            commands::agent_docs::agent_docs_strategy_set,
            commands::agent_docs::agent_docs_fix_plan,
            commands::agent_docs::agent_docs_fix_apply,
            commands::agent_docs::agent_docs_resolve,
            commands::snippets::snippets_list,
            commands::snippets::snippet_show,
            commands::snippets::snippet_new,
            commands::snippets::snippet_edit,
            commands::snippets::snippet_delete,
            commands::snippets::snippet_apply,
            commands::snippets::snippet_remove,
            commands::snippets::snippet_update,
            commands::snippets::snippet_status,
            commands::permissions::permissions_show,
            commands::permissions::permissions_set,
            commands::permissions::permissions_validate,
            commands::permissions::permissions_capabilities,
            commands::permissions::permissions_doctor,
            commands::permissions::permissions_adopt,
            commands::permissions::permissions_import_candidates,
            commands::permissions::permissions_import_apply,
            commands::permissions::permissions_reconcile_candidates,
            commands::permissions::permissions_reconcile_apply,
            commands::permissions::permissions_disable,
            commands::permissions::permissions_migrate_scope,
            commands::permissions::permissions_risks_schema,
            commands::permissions::permissions_recent_imports,
            commands::remotes::remote_list,
            commands::remotes::remote_connectors,
            commands::remotes::remote_show,
            commands::remotes::remote_diff,
            commands::remotes::remote_health,
            commands::remotes::remote_fetch_doc,
            commands::remotes::remote_list_docs,
            commands::remotes::remote_push_doc,
            commands::remotes::remote_doctor,
            commands::remotes::remote_scan_imports,
            commands::remotes::remote_add,
            commands::remotes::remote_sync,
            commands::remotes::remote_resolve,
            commands::remotes::remote_disable,
            commands::remotes::remote_enable,
            commands::remotes::remote_set_apply_global,
            commands::remotes::remote_remove,
            commands::remotes::remote_clear,
            commands::remotes::remote_import_skill,
            commands::remotes::remote_set_secret,
            commands::remotes::remote_has_secret,
            commands::remotes::remote_delete_secret,
            commands::remotes::remote_fetch_host_key,
            commands::remotes::remote_setup_key,
            commands::remotes::remote_pin,
            commands::remotes::remote_probe,
            commands::subagents::subagent_list,
            commands::subagents::subagent_show,
            commands::subagents::subagent_save,
            commands::subagents::subagent_delete,
            commands::subagents::subagent_set_disabled,
            commands::subagents::subagent_skill_usage,
            commands::subagents::subagent_attachable_skills,
            commands::subagents::subagent_link,
            commands::subagents::subagent_unlink,
            commands::subagents::subagent_link_status,
            commands::subagents::subagent_resolve_drift,
            commands::subagents::subagent_provision_skill,
            commands::hooks::hook_list,
            commands::hooks::hook_show,
            commands::hooks::hook_capabilities,
            commands::hooks::hook_new,
            commands::hooks::hook_edit,
            commands::hooks::hook_delete,
            commands::hooks::hook_attach,
            commands::hooks::hook_detach,
            commands::hooks::hook_set_settings,
            commands::usage::usage_scan_ccusage,
            commands::usage::usage_load_latest_ccusage,
            commands::backup::backup_status,
            commands::backup::backup_auth_status,
            commands::backup::backup_auth_login_pat,
            commands::backup::backup_auth_logout,
            commands::backup::backup_init,
            commands::backup::backup_now,
            commands::backup::backup_enable,
            commands::backup::backup_disable,
            commands::backup::restore_preview,
            commands::backup::restore_apply,
        ])
        .run(tauri::generate_context!())
        .expect("error while running tauri application")
}
