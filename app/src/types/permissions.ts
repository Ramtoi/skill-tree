...
> = {
	// Codex and opencode are Bash-only: a non-Bash pattern (path Read/Write,
	// WebFetch, mcp__*) cannot be expressed and is skipped by their adapters.
	codex: (rule) => bashPrefixTokens(rule.pattern) !== null,
	opencode: (rule) => bashPrefixTokens(rule.pattern) !== null,
};
...
export function stripResolverFields(
	...
	return {
		...
		// hooks are being retired from the permissions payload (authored on the
		// Hooks surface now); tolerate a payload that omits the key entirely.
