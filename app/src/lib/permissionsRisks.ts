...
const PREDICATES: Record<string, Predicate> = {
	...
	// An allow and a deny share the same pattern → deny wins at runtime, the
	// allow is dead. Mirrors risks.py `_pred_contradictory_rule`.
	CONTRADICTORY_RULE: (p) => {
		const denyPatterns = new Set(p.deny.map((r) => r.pattern));
		return Array.from(
			new Set(
				p.allow.filter((r) => denyPatterns.has(r.pattern)).map((r) => r.pattern),
			),
		).sort();
	},
	// DROPPED_DENY / DROPPED_HOOK are raised at translate-time by Bash-only
	// adapters (Codex, opencode) when they cannot express a deny/ask or hook —
	// they need harness context, so they are NOT evaluated client-side from a
	// plain NormalizedPermissions. Their label/severity still ship in the schema
	// (risks.py emit_schema) for display when the engine surfaces them.
};
