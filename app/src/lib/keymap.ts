...
export const KEYMAP: KeyBinding[] = [
  ...
  },
  {
    ...
    group: "Navigation",
    hint: "g k",
    run: (ctx) => ctx.navigate("/hooks"),
  },
  {
    id: "nav.usage",
    keys: ["g", "u"],
    label: "Go to Local Agent Usage",
    group: "Navigation",
    hint: "g u",
    run: (ctx) => ctx.navigate("/usage"),
  },
  {
    id: "nav.permissions",
    keys: ["g", "P"],
  ...
  {
    // `g u` (the design's first suggestion) was already bound to nav.usage.
    // Shifted `b` keeps the Backup mnemonic without colliding with `g b`
    // (Bundles) — the same shift-disambiguation `g ⇧p` uses against `g p`.
    id: "nav.backup",
    keys: ["g", "B"],
    label: "Go to Backup",
    group: "Navigation",
    hint: "g ⇧b",
    run: (ctx) => ctx.navigate("/backup"),
  },
  // ── Create (c-prefix) ──
  {
    id: "create.skill",
