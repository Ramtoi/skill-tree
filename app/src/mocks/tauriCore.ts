...
function stubDir(name: string, child: { name: string; path: string; dirs: unknown[]; files: unknown[] }) {
  return { name, path: "", dirs: [child], files: [] };
}
const androidPresentation = {
  name: "presentation",
  path: `${AD_BASE}/app/src/main/java/com/notesapp/presentation`,
  files: [],
  dirs: [
    {
      name: "board",
      path: `${AD_BASE}/app/src/main/java/com/notesapp/presentation/board`,
      dirs: [],
      files: [adFile("app/src/main/java/com/notesapp/presentation/board/README.md")],
    },
    {
      name: "capture",
      path: `${AD_BASE}/app/src/main/java/com/notesapp/presentation/capture`,
      dirs: [],
      files: [adFile("app/src/main/java/com/notesapp/presentation/capture/README.md")],
    },
  ],
};
const androidAppDir = stubDir(
  "app",
  stubDir("src", stubDir("main", stubDir("java", stubDir("com", stubDir("notesapp", androidPresentation))))),
);
...
if (typeof window !== "undefined") {
  try {
    if (new URLSearchParams(window.location.search).get("libraryEmpty") === "1") {
...
// default populated mock is untouched for every other scene. Reads the same
// `window.location.search` channel as `ipcDelayMs()`.
function sceneFlag(name: string): boolean {
...
function visualUsageScan({ empty = false, totalTokens = 128_450, includeCodex = true } = {}) {
  if (empty) {
    return {
  ...
  return {
    ...
    parsed: {
      daily: [
        {
          ...
          totalCost: 1.92,
          agents: [
            { agent: "claude", inputTokens: 13_000, outputTokens: 3_000, cacheCreationTokens: 1_000, cacheReadTokens: 1_500, totalTokens: 18_500, totalCost: 1.92, modelsUsed: ["claude-sonnet-4"] },
          ],
        },
        {
          period: "2026-07-13",
          ...
          totalCost: 5.14,
          agents: [
            { agent: "claude", inputTokens: 30_000, outputTokens: 6_000, cacheCreationTokens: 2_000, cacheReadTokens: 3_250, totalTokens: 41_250, totalCost: 5.14, modelsUsed: ["claude-sonnet-4"] },
          ],
        },
        {
          period: "2026-07-14",
          ...
          totalCost: 17.04,
          agents: [
            { agent: "claude", inputTokens: 74_000, outputTokens: 16_400, cacheCreationTokens: 6_100, cacheReadTokens: 3_500, totalTokens: 100_000, totalCost: 14.28, modelsUsed: ["claude-sonnet-4"] },
            ...(includeCodex ? [{ agent: "codex", inputTokens: 19_200, outputTokens: 6_250, cacheCreationTokens: 1_100, cacheReadTokens: 1_900, totalTokens: 28_450, totalCost: 2.76, modelsUsed: ["gpt-5.5"] }] : []),
          ],
        },
      ],
      weekly: [],
      monthly: [],
      session: sessions,
      totals: { inputTokens: 93_200, outputTokens: 22_650, cacheCreationTokens: 7_200, cacheReadTokens: 5_400, totalTokens, totalCost: 17.04 },
    },
  };
}
...
/** Strip the store's internal `project_settings` for a `hook list` row. */
function toHookRow(h: MockHookDef) {
  const { project_settings: _ps, ...row } = h;
  void _ps;
  return row;
}
...
function dispatch(cmd: string, args?: Record<string, unknown>): unknown {
  ...
  switch (cmd) {
    ...
    case "hook_list":
      ...
      return {
        hooks: hooksStore.map(toHookRow),
        reach: HOOK_REACH_DEFAULT,
      };
    case "hook_capabilities":
      return sceneFlag("hookCapsVaried") ? hookCapsVaried : hookCapsDefault;
    case "hook_show": {
      const h = findMockHook(args?.name as string) ?? hooksStore[0];
      return {
        ...toHookRow(h),
    ...
    case "list_agent_docs":
      return args?.includeAllMarkdown ? agentDocsMarkdownListing : agentDocsListing;
    case "read_agent_doc": {
      const adRel = (args?.relativePath as string) ?? "CLAUDE.md";
      return {
        rel: adRel,
        absolute_path: `${AD_BASE}/${adRel}`,
        content: `# ${adRel.split("/").pop() ?? adRel}\n\nProject documentation.\n`,
        size: 40,
        modified_at: null,
        hash: "deadc0de",
        is_symlink: false,
    ...
    case "backup_init":
      return {
        ...
        branch: "main",
        created: true,
        create_detail: "created private repo me/skill-hub-backup",
        auth: "ssh",
        gh_login: "me",
        enabled: true,
        initialized: true,
...
function mockBackupStatus() {
  const configured = !sceneFlag("backupUnconfigured");
  ...
  return {
    ...
    configured,
    dir: "~/.skill-hub-backup",
    remote: configured ? "git@github.com:me/skill-hub-backup.git" : null,
    repo: configured ? "me/skill-hub-backup" : null,
    ...
    manifest: configured
      ...
      : null,
    warnings: stale
      ? ["4 consecutive push failures — the cloud copy is stale"]
      : [],
  };
}
  ...
  return {
    ...
    ladder: [
      ...
      },
      {
        method: "gh",
        available: true,
        detail: "gh CLI authenticated as me",
        user: "me",
      },
      {
        method: "pat",
...
function mockRestorePlan(applied: boolean, args?: unknown) {
  ...
  const source = req.source || "git@github.com:me/skill-hub-backup.git";
  ...
  const head = {
    ...
    apply: applied,
    source,
    snapshot_dir: "~/.skill-hub/state/restore-cache/skill-hub-backup-1a2b3c4d5e",
