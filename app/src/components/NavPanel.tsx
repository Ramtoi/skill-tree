...
import { projectFreshness, type SyncReportEnvelope } from "@/lib/syncFreshness";
import { syncMinutes } from "@/lib/syncMinutes";
import { deriveSources } from "@/lib/skillSource";
import type { RecentItem, Registry, SourceStatus, SourceView } from "@/types";
...
interface SideSectionProps {
  ...
  onAdd?: () => void;
  addTitle?: string;
  /** Optional `data-tour` id set on the add ("+") button (tips tour anchor). */
  addTourId?: string;
  variant?: "featured";
  quiet?: boolean;
  summary?: string;
  ...
  for (const k of pinned) {
    ...
    if (kind === "project") {
      ...
      out.push({
        ...
        id,
        name: id,
        count: resolveActiveSkills(proj, registry).length,
        leading: <FreshnessDot state={projectFreshness(id, syncEnvelope)} size={6} />,
      });
    } else if (kind === "bundle") {
...
export function NavPanel() {
  ...
  // Same frontend-derivable view the Sources screen uses (built-in Local +
  // Starter Pack, plus any registered git/litellm sources) — no extra backend
  // call, just the registry already loaded for this route (C4).
  const sources: SourceView[] = useMemo(() => deriveSources(registry), [registry]);
  ...
  const pinnedItems = useMemo(
    () => resolvePinned(pinned, registry, sources, syncEnvelope),
    [pinned, registry, sources, syncEnvelope],
  );

  // Resolved active-skill count per project (matches the workspace badge).
  const projectSkillCounts = useMemo(() => {
    const m = new Map<string, number>();
    if (registry) {
      for (const [name, proj] of Object.entries(registry.projects)) {
        m.set(name, resolveActiveSkills(proj, registry).length);
      }
    }
    return m;
  }, [registry]);
      ...
      <div className="side-scroll">
        ...
        >
          {projSlice.visible.map((p) => {
            ...
            const eqCount = projectSkillCounts.get(p.name) ?? 0;
            return (
              <SideRow
                key={p.name}
                active={active}
                ...
                leading={
                  <FreshnessDot
                    state={projectFreshness(p.name, syncEnvelope)}
                    size={6}
                  />
                }
                name={p.name}
