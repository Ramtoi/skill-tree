...
export function AgentDocsFixBanner({
	...
	async function openPlan() {
		...
		try {
			...
			toast.error("Could not build the fix plan", String(err));
		} finally {
			setBusy(false);
		}
	}
	...
	async function applyPlan() {
		...
		try {
			...
			if (!res.applied) {
				...
				toast.error("Fix failed", res.error ?? "unknown error");
				return;
			}
	...
	async function openCompare() {
		...
		try {
			...
			toast.error("Could not load both root files", String(err));
		} finally {
			setBusy(false);
		}
	}
	...
	async function resolve(op: AgentDocResolveOp) {
		...
		try {
			...
			if (!res.applied) {
				toast.error("Could not resolve", res.error ?? "unknown error");
				return;
			}
			toast.push({
				kind: "success",
			...
			toast.error("Could not resolve", String(err));
		} finally {
			setBusy(false);
		}
	}
