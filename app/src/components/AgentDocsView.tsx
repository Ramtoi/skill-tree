...
/** True if a folder or any of its descendants holds an agent-instruction
 *  basename (`AGENT_DOC_BASENAMES`). Drives a hint on a collapsed folder row
 *  in the all-Markdown tree so a nested CLAUDE.md/AGENTS.md doesn't go
 *  unnoticed just because its folder hasn't been expanded yet. */
function treeHasAgentDoc(node: AgentDocFolder): boolean {
	if (node.files.some((f) => AGENT_DOC_BASENAMES.has(f.name))) return true;
	for (const d of node.dirs) if (treeHasAgentDoc(d)) return true;
	return false;
}

/** Walk down through a chain of pass-through directories — no files of their
 *  own, exactly one subdirectory (an Android `app/src/main/java/com/pkg/`
 *  package path is the textbook case) — and collapse it into one row. One
 *  click on the combined name jumps straight to the first folder that
 *  actually has something in it, instead of forcing a click per empty
 *  stub level on the way down. Folders with real content (files, or more
 *  than one subdirectory) are returned unchanged. */
function collapseStubChain(dir: AgentDocFolder): {
	name: string;
	folder: AgentDocFolder;
} {
	let name = dir.name;
	let folder = dir;
	while (folder.files.length === 0 && folder.dirs.length === 1) {
...
const LEGACY_BASENAME = "AGENT.md";

/** Basenames the backend treats as agent-instruction files (`ALLOWED_BASENAMES`
 *  in `agent_docs.rs`), at any depth — a root or nested `CLAUDE.md`/`AGENTS.md`
 *  is structurally different from an arbitrary project doc. The flat
 *  all-Markdown tree renders every `.md` file identically otherwise, so
 *  these need their own marker to read as agent docs rather than one of
 *  hundreds of look-alike rows. */
const AGENT_DOC_BASENAMES = new Set(["CLAUDE.md", "AGENTS.md", LEGACY_BASENAME]);
...
function FolderRow({
	...
	/** All-Markdown tree only: a nested CLAUDE.md/AGENTS.md/AGENT.md is hiding
	 *  inside this still-collapsed folder. */
	hasAgentDoc?: boolean;
}) {
	return (
		<button
		...
		>
			...
			{hasAgentDoc && !expanded && (
				<Icon
					name="doc"
					size={9}
					tone="blue"
					title="Contains an agent instruction file"
...
function FileTree({
	...
	return (
		<>
			...
			{dirs.map((dir) => {
				// A directory chain that's just pass-through stubs (no files,
				// exactly one subdirectory each — an Android package path is
				// the textbook case) collapses into one row; the state key,
				// content checks, and expanded contents all key off the first
				// folder that actually has something in it.
				const { name: displayName, folder: target } = collapseStubChain(dir);
				const subPath = target.path;
				// Unlike the instruction-set map (a handful of curated sets),
				// the flat all-Markdown tree can surface hundreds of nested
				// files across many directories — auto-expanding all of them
				// produced an overwhelming, fully-unrolled wall. Folders here
				// start collapsed; the user drills in explicitly.
				const isOpen = expanded[subPath] === true;
				const subAllMissing = !treeHasExistingFile(target);
				const subIsKnown = KNOWN_RELS.some((k) => k.startsWith(`${subPath}/`));
				const hasDirty = treeHasDirty(target, isDirtyOf);
				const hasAgentDoc = treeHasAgentDoc(target);
				return (
					<div key={`d:${subPath}`}>
						<FolderRow
							name={displayName}
							depth={depth + 1}
							expanded={isOpen}
							onToggle={() => toggleExpanded(subPath, isOpen)}
				...
				return (
					<div key={`d:${child.fullPath}`}>
						<FolderRow
							...
							depth={depth + 1}
							expanded={isOpen}
							onToggle={() => toggleExpanded(key, isOpen)}
							allMissing={false}
							isKnown={false}
							hasDescendantDirty={hasDirty}
						/>
...
export function AgentDocsView({
	...
	// Flips relative to the caller's own computed open state rather than
	// re-deriving a default here, since FileTree and InstructionSetTree read
	// this same map with different default semantics (collapsed vs expanded).
	function toggleExpanded(path: string, currentlyOpen: boolean) {
