...
export function StatusBar() {
  ...
  // Backup staleness is MANDATORY-visible (design §9): a fail-open backup pass
  // must not be fail-SILENT. The chip renders only when there is something
  // wrong — consecutive push failures, a pending restore reconcile, or a
  // refused publish recorded by the last sync's tail pass.
  const { data: backupStatus } = useBackupStatus();
  const backupSlot = (
    syncEnvelope?.report?.global as unknown as
      | { backup?: SyncReportBackupSlot }
      | undefined
  )?.backup;
  const backupAlert = backupWarning(backupStatus, backupSlot);
  ...
  return (
    <div className="app-status">
      ...
      {backupAlert.level !== "none" && (
        <button
          type="button"
          className="status-segment clickable backup-chip"
          data-backup-state={backupAlert.level}
          data-testid="backup-chip"
          title={backupAlert.detail}
          onClick={() => navigate("/backup")}
          style={{
            color:
              backupAlert.level === "danger" ? "var(--red)" : "var(--amber)",
          }}
        >
          <Icon name="warning" size={11} />
          {/* Wrapped so the label can ellipsize at narrow widths: the chip is
              mandatory-visible, but its label is the longest in the bar and
              must not evict the ⌘K trigger to stay whole. */}
          <span className="backup-chip-label">{backupAlert.label}</span>
        </button>
      )}
