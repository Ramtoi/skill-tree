...
function AppShell() {
  ...
  const { data: pythonOk, isLoading } = useQuery({
    queryKey: ["python"],
    queryFn: () => invoke<boolean>("check_python"),
    staleTime: Infinity,
  });

  const { data: bootstrapState, isLoading: bootstrapLoading } = useQuery({
    queryKey: ["bootstrap"],
    queryFn: () => invoke<BootstrapState>("bootstrap_check"),
    enabled: pythonOk === true,
    staleTime: 5 * 60_000,
  });
  ...
  // Bootstrap takes precedence over routes (but Python gate still runs first)
  if (pythonOk === true && bootstrapState?.needs_bootstrap && !degradedMode) {
    return (
      <div
        className="app"
        data-rail="false"
  ...
  const showRoutes = pythonOk === true || degradedMode;
