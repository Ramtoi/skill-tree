...
describe("StatusBar global busy indicator", () => {
  ...
  it("appears after a debounce, gains a 5s hint, and clears when idle", async () => {
    ...
    // A "has synced" report so the drained-idle assertion below exercises the
    // green "in sync" branch rather than the honest not-synced-yet default.
    // Must mock the invoke response too (not just seed the cache) — the
    // background refetch (staleTime: 0) would otherwise clobber it with the
    // default mock's `null` by the time the `waitFor` below settles.
