...
describe("Snippets screen", () => {
	...
	// B4b-01: the master/detail collapse contract. `.snip-screen[data-view]`
	// drives the narrow-width single-pane CSS, and a back affordance (visible only
	// once the list column collapses) returns to the list.
	it("reflects list/detail view state and offers a back-to-list affordance", async () => {
		const { container } = renderWithProviders(<Snippets />);
		const screenEl = () => container.querySelector(".snip-screen");

		await screen.findByText("validation-procedure");
		// Nothing selected → list view, no back button.
		expect(screenEl()?.getAttribute("data-view")).toBe("list");
		expect(screen.queryByRole("button", { name: /Snippets/ })).toBeNull();

		// Selecting a snippet flips to the detail view + renders the back button.
		fireEvent.click(screen.getByText("validation-procedure"));
		await waitFor(() =>
			expect(screenEl()?.getAttribute("data-view")).toBe("detail"),
		);
		const back = await screen.findByRole("button", { name: /Snippets/ });
