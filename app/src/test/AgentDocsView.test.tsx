...
// The edit surface is a CodeMirror 6 editor (one layer — caret can't drift).
// Set its document by dispatching a transaction on the live EditorView, which
// fires the same updateListener path real typing does.
function editorView(): EditorView {
	const dom = document.querySelector(
		".agent-docs-editor .cm-editor",
	) as HTMLElement | null;
...
/** Current document text of the live CodeMirror editor (replaces the old
    getByDisplayValue read against the textarea). */
...
function mdFile(rel: string, hash: string) {
	return {
		rel,
		name: rel.split("/").pop() ?? rel,
		label: rel,
...
/** Build the `includeAllMarkdown: true` response from a base listing: adds a
 *  root `README.md` and a nested `docs/guides/setup.md`, and clears
 *  instruction_sets (the backend returns none in all-Markdown mode). */
function withMarkdownFiles(base: AgentDocsListing): AgentDocsListing {
	return listing({
		...base,
		instruction_sets: [],
		all_rels: [...base.all_rels, "README.md", "docs/guides/setup.md"],
		root: {
			...base.root,
			files: [...base.root.files, mdFile("README.md", "hashReadme")],
			dirs: [
				...base.root.dirs,
				{
					name: "docs",
					...
					dirs: [
						{
							...
							path: "docs/guides",
							dirs: [],
							files: [mdFile("docs/guides/setup.md", "hashSetup")],
						},
					],
				},
			],
...
/** All-Markdown tree folders default to collapsed; drill into `docs/guides/`
 *  the way a user would, one folder click at a time. Waits for the folder
 *  row itself, since the "Markdown files" header is derived from local
 *  toggle state and can render a tick before the tree's own data has. */
...
describe("AgentDocsView", () => {
	it("toggle lists all project Markdown files and edits them through the same save flow", async () => {
		...
		const markdownListing = withMarkdownFiles(defaultListing);
		setupInvoke({
		...
		// Folders start collapsed. docs/ has no files of its own and exactly
		// one subdirectory (guides/), so it's a pass-through stub that
		// collapses into a single combined row — one click reaches setup.md.
		expect(screen.queryByText("setup.md")).not.toBeInTheDocument();
		await expandFolder("docs/guides");
		await waitFor(() => expect(screen.getByText("setup.md")).toBeInTheDocument());
	...
	it("collapses all-Markdown folders by default and hints at a nested agent doc hiding inside one", async () => {
		const defaultListing = listing();
		const markdownListing = withMarkdownFiles(defaultListing);
		// docs/ directly holds a nested CLAUDE.md (agent doc) alongside the
		// existing docs/guides/setup.md (an ordinary doc, one level deeper).
		const docsDir = markdownListing.root.dirs.find((d) => d.name === "docs")!;
		markdownListing.root = {
			...markdownListing.root,
			dirs: markdownListing.root.dirs.map((d) =>
				d === docsDir
					? { ...docsDir, files: [mdFile("docs/CLAUDE.md", "hashDocsClaude")] }
					: d,
			),
		};
		markdownListing.all_rels = [...markdownListing.all_rels, "docs/CLAUDE.md"];
		setupInvoke({
			list_agent_docs: (args) =>
				args.includeAllMarkdown ? markdownListing : defaultListing,
			read_agent_doc: (rel) => content(rel, "# stub\n", rel),
		});
		...
		fireEvent.click(screen.getByTestId("agent-docs-show-all-markdown"));
		await waitFor(() => expect(screen.getByText("Markdown files")).toBeInTheDocument());

		// Collapsed by default: neither nested file shows until docs/ is opened.
		// (The root's own CLAUDE.md always renders — it's not inside a folder —
		// so the nested one is distinguished by count, not presence.)
		await waitFor(() => expect(screen.getByText("docs/")).toBeInTheDocument());
		expect(screen.queryByText("setup.md")).not.toBeInTheDocument();
		expect(screen.getAllByText("CLAUDE.md", { selector: ".ad-file-name" })).toHaveLength(1);
		// Root CLAUDE.md + root AGENTS.md (both always visible, not folder-gated).
		expect(screen.getAllByText("AGENT DOC")).toHaveLength(2);

		// The still-collapsed docs/ row hints that an agent doc is inside.
		const docsRow = screen.getByText("docs/").closest("button") as HTMLElement;
		expect(within(docsRow).getByTitle("Contains an agent instruction file")).toBeInTheDocument();

		await expandFolder("docs");
		await waitFor(() =>
			expect(screen.getAllByText("CLAUDE.md", { selector: ".ad-file-name" })).toHaveLength(2),
		);
		// Expanded: the hint is gone (the row itself now carries an AGENT DOC pill).
		expect(
			within(screen.getByText("docs/").closest("button") as HTMLElement).queryByTitle(
				"Contains an agent instruction file",
			),
		).not.toBeInTheDocument();
		// The newly-revealed nested CLAUDE.md now also carries the AGENT DOC
		// marker, on top of the two always-visible root files.
		expect(screen.getAllByText("AGENT DOC")).toHaveLength(3);
	});

	it("collapses a pass-through directory chain (e.g. an Android package path) into one click", async () => {
		const defaultListing = listing();
		const markdownListing = withMarkdownFiles(defaultListing);
		// app/src/main/java/com/notesapp/presentation/ — six directories deep,
		// each holding no files of its own and exactly one subdirectory, until
		// presentation/ finally has real content (board/, plus a file).
		const stub = (name: string, child: AgentDocFolder): AgentDocFolder => ({
			name,
			path: "",
			files: [],
			dirs: [child],
		...
		const board = {
			name: "board",
			path: "app/src/main/java/com/notesapp/presentation/board",
			files: [mdFile("app/src/main/java/com/notesapp/presentation/board/README.md", "hashBoard")],
			dirs: [],
		};
		const capture = {
			name: "capture",
			path: "app/src/main/java/com/notesapp/presentation/capture",
			files: [mdFile("app/src/main/java/com/notesapp/presentation/capture/README.md", "hashCapture")],
			dirs: [],
		};
		// presentation/ has two subdirectories (board/, capture/) — real
		// content, not a pass-through stub — so the chain stops here.
		const presentation = {
			name: "presentation",
			path: "app/src/main/java/com/notesapp/presentation",
			files: [],
			dirs: [board, capture],
		};
		const app = stub(
			"app",
			stub("src", stub("main", stub("java", stub("com", stub("notesapp", presentation))))),
		);
		markdownListing.root = { ...markdownListing.root, dirs: [...markdownListing.root.dirs, app] };
		setupInvoke({
			list_agent_docs: (args) =>
				args.includeAllMarkdown ? markdownListing : defaultListing,
			read_agent_doc: (rel) => content(rel, "# stub\n", rel),
		...
		// The whole stub chain reads as one combined row, not six separate ones.
		expect(
			screen.getByText("app/src/main/java/com/notesapp/presentation/"),
		).toBeInTheDocument();
		expect(screen.queryByText("app/")).not.toBeInTheDocument();
		expect(screen.queryByText("src/")).not.toBeInTheDocument();

		// One click on the combined row reaches presentation/'s real content —
		// board/ — directly, without a click per empty stub level.
		fireEvent.click(screen.getByText("app/src/main/java/com/notesapp/presentation/"));
		await waitFor(() => expect(screen.getByText("board/")).toBeInTheDocument());
	});

	it("flags a legacy AGENT.md row in the raw Markdown tree so it doesn't look like an unexplained duplicate of AGENTS.md", async () => {
		const defaultListing = listing();
		const markdownListing = withMarkdownFiles(defaultListing);
		const legacyAgentFile = {
			...mdFile("AGENT.md", "hashAgentLegacy"),
			is_symlink: true,
			symlink_to: "CLAUDE.md",
			symlink_target_in_project: true,
			can_write: false,
		};
		markdownListing.all_rels = [...markdownListing.all_rels, "AGENT.md"];
		markdownListing.root = {
			...markdownListing.root,
			files: [...markdownListing.root.files, legacyAgentFile],
		};
		setupInvoke({
			list_agent_docs: (args) =>
				args.includeAllMarkdown ? markdownListing : defaultListing,
		...
		const agentRow = screen.getByText("AGENT.md").closest("button");
		expect(agentRow).not.toBeNull();
		expect(within(agentRow as HTMLElement).getByText("LEGACY")).toBeInTheDocument();

		for (const claudeRow of screen.getAllByText("CLAUDE.md")) {
			const row = claudeRow.closest("button");
			expect(row).not.toBeNull();
			expect(within(row as HTMLElement).queryByText("LEGACY")).not.toBeInTheDocument();
		}
	});

	it("switches from the instruction-set map to the raw Markdown tree when toggled on", async () => {
		const setsListing = deviatingListing();
		const markdownListing = withMarkdownFiles(setsListing);
		setupInvoke({
			list_agent_docs: (args) =>
				args.includeAllMarkdown ? markdownListing : setsListing,
			read_agent_doc: (rel) => content(rel, "# Setup\n", rel),
		});

		renderView();
		// Default mode renders the instruction-set map (set label + "Instruction map").
		await waitFor(() => {
			expect(screen.getByText("Instruction map")).toBeInTheDocument();
		...
		// All-Markdown mode replaces the set map with the raw file tree and the
		// count switches from "N sets" to the "N files" form.
		await waitFor(() => {
			expect(screen.getByText("Markdown files")).toBeInTheDocument();
		});
		expect(screen.queryByText("Instruction map")).not.toBeInTheDocument();
		expect(screen.queryByText("Project Instructions")).not.toBeInTheDocument();
		expect(screen.getByText(/^\d+ files$/)).toBeInTheDocument();

		// Folders start collapsed. docs/ is a pass-through stub (no files of
		// its own, one subdirectory) so it collapses with guides/ into one
		// combined row — one click reaches setup.md.
		expect(screen.queryByText("setup.md")).not.toBeInTheDocument();
		await expandFolder("docs/guides");
		await waitFor(() => expect(screen.getByText("setup.md")).toBeInTheDocument());
	});

	it("replaces the canonical unified root with the raw tree in all-Markdown mode", async () => {
		const canonical = canonicalListing();
		const markdownListing = withMarkdownFiles(canonical);
		setupInvoke({
			list_agent_docs: (args) =>
				args.includeAllMarkdown ? markdownListing : canonical,
			read_agent_doc: (rel) => content(rel, "# Setup\n", rel),
		});

		renderView();
		// Canonical derived root → instruction-set map (unified root), not raw files.
		await waitFor(() =>
			expect(
				document.querySelector(".ad-instruction-set-list"),
			).not.toBeNull(),
		);
		expect(screen.queryByText("setup.md")).not.toBeInTheDocument();
		...
		// The unified-root instruction map is gone; the raw file tree is shown.
		expect(document.querySelector(".ad-instruction-set-list")).toBeNull();

		// Folders start collapsed; docs/ + guides/ collapse into one stub-chain row.
		await expandFolder("docs/guides");
		await waitFor(() => expect(screen.getByText("setup.md")).toBeInTheDocument());
	});

	it("restores standard Agent Docs mode and resets selection when toggled back off", async () => {
		const setsListing = deviatingListing();
		const markdownListing = withMarkdownFiles(setsListing);
		setupInvoke({
			list_agent_docs: (args) =>
		...
		renderView();
		const toggle = () =>
			screen.getByTestId("agent-docs-show-all-markdown");
		await waitFor(() => expect(toggle()).toBeInTheDocument());

		// On → open a Markdown-only file (folders start collapsed; docs/ +
		// guides/ collapse into one stub-chain row — drill in with one click).
		...
		// Off → instruction map restored, Markdown-only row gone, and the prior
		// selection cleared (the editor body only mounts when a file is selected).
		fireEvent.click(toggle());
		await waitFor(() => {
			expect(screen.getByText("Instruction map")).toBeInTheDocument();
		});
		...
		expect(
			document.querySelector(".agent-docs-editor .cm-editor"),
		).toBeNull();
	});

	it("hides the Agent-Docs-only snippet strip while browsing arbitrary Markdown", async () => {
		const setsListing = deviatingListing();
		const markdownListing = withMarkdownFiles(setsListing);
		setupInvoke({
			list_agent_docs: (args) =>
		...
		// The snippet strip is agent-doc-only; setup.md satisfies every other
		// render condition, so its absence proves the browse-mode gate holds.
		expect(screen.queryByText("Applied snippets")).not.toBeInTheDocument();
	});

	it("clears all-Markdown mode when the project changes", async () => {
		const setsListing = deviatingListing();
		const markdownListing = withMarkdownFiles(setsListing);
		setupInvoke({
			list_agent_docs: (args) =>
		...
		const { client, rerender } = renderWithProviders(view("/p"));
		primeRegistry(client);
		...
		fireEvent.click(screen.getByTestId("agent-docs-show-all-markdown"));
		await waitFor(() => expect(screen.getByText("Markdown files")).toBeInTheDocument());
		expect(screen.getByTestId("agent-docs-show-all-markdown")).toBeChecked();

		// Switching projects resets the toggle back to standard Agent Docs mode.
		rerender(view("/p2"));
		await waitFor(() => expect(screen.getByText("Instruction map")).toBeInTheDocument());
		expect(screen.getByTestId("agent-docs-show-all-markdown")).not.toBeChecked();
	});
	...
	it("shows file map count and known + discovered files", async () => {
		...
		// Discovered nested folder visible — core/ has no files of its own and
		// exactly one subdirectory, so it collapses with canvas/ into one row.
		expect(screen.getByText("core/canvas/")).toBeInTheDocument();
	});
	...
	it("renders content after lazy read; edit marks UNSAVED; Save calls write_agent_doc", async () => {
		...
		renderView();
		await waitFor(() => {
			expect(editorText()).toMatch(/# original/);
		});
	...
	it("grounds instruction-set selection in the canonical real file", async () => {
		...
		renderView();
		await waitFor(() => {
			expect(editorText()).toMatch(/# CLAUDE.md/);
		});
		...
		await waitFor(() => {
			expect(editorText()).toMatch(/# core\/canvas\/AGENTS.md/);
		});
