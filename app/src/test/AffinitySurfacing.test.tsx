...
describe("Affinity mismatch surfacing (M8)", () => {
	it("badges an equipped skill whose affinity excludes the project's effective harnesses", () => {
		renderAlpha();
		// codex-only won't sync (project effective = [claude-code]). The badge is
		// icon-only; its meaning lives in the accessible name (B5-04).
		expect(screen.getByLabelText(/won't sync here/i)).toBeInTheDocument();
	});

	it("does not badge a skill with no affinity", () => {
		renderAlpha();
		// Only ONE badge — the codex-only one; portable1 has no affinity.
		expect(screen.getAllByLabelText(/won't sync here/i)).toHaveLength(1);
	});
	...
	it("shows the project banner when the sync report records affinity_skips", () => {
		...
		expect(screen.getByText(/won't reach any harness/i)).toBeInTheDocument();
		expect(
	...
	it("shows no banner when the report has no affinity_skips", () => {
		renderAlpha();
		expect(screen.queryByText(/won't reach any harness/i)).toBeNull();
	});
});
