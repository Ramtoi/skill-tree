...
describe("IconRail", () => {
  it("shows logo and the eight destinations + two utilities", () => {
  ...
  it("places the Hooks destination between Snippets and Permissions", () => {
    const client = makeQueryClient();
    primeRegistry(client);
    ...
    const titleOrder = buttons.map((b) => b.getAttribute("title"));
    const snippets = titleOrder.indexOf("Snippets");
    const hooks = titleOrder.indexOf("Hooks");
    const permissions = titleOrder.indexOf("Permissions");
    expect(snippets).toBeGreaterThanOrEqual(0);
    expect(hooks).toBeGreaterThan(snippets);
    expect(permissions).toBeGreaterThan(hooks);
  });
