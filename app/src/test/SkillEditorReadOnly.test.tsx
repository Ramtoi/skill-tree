...
describe("SkillEditor — hybrid ownership", () => {
  ...
  it("locks the markdown textarea read-only for externally managed skills", async () => {
    const { container } = renderEditor("/skill/android-compose-ui");
    await waitFor(() => container.querySelector(".code-area textarea"));
    const ta = container.querySelector(".code-area textarea") as HTMLTextAreaElement | null;
