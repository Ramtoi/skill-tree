...
describe("screen layout contract (CSS)", () => {
	...
	it("defines the state-pill variants", () => {
		expect(css).toMatch(/\.state-pill\[data-state="unsaved"\]/);
		expect(css).toMatch(/\.state-pill\[data-state="saved"\]/);
	});
	...
	it("pins the edit-mode textarea to the content-sized overlay wrapper", () => {
		expect(rule(".code-area-inner")).toMatch(/position:\s*relative/);
		const ta = rule(".code-area--edit textarea");
		expect(ta).toMatch(/position:\s*absolute/);
		expect(ta).toMatch(/overflow:\s*hidden/);
		expect(ta).toMatch(/white-space:\s*pre-wrap/);
		// pre and textarea must wrap identically
		expect(rule(".code-area--edit pre")).toMatch(/white-space:\s*pre-wrap/);
	});
