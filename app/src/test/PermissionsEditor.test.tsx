...
describe("PermissionsEditor redesigned surface", () => {
	...
	it("adds alternate entry types from the split add menu and focuses the row", async () => {
		...
		// offers Allow / Deny / Ask (hooks moved to the /hooks surface).
		await screen.findAllByText("ALLOW");
		fireEvent.click(screen.getByLabelText("Choose permission type"));
		// Hook authoring is gone — the add menu never offers it.
		expect(screen.queryByText("Add hook")).toBeNull();
		fireEvent.click(screen.getByText("Add deny"));
		const pattern = await screen.findByLabelText("Pattern");
		expect(pattern).toHaveFocus();
		// The DENY filter reflects aria-pressed=true after we add a deny rule.
		const denyPressed = screen
			.getAllByRole("button", { pressed: true })
			.filter((b) => /DENY/.test(b.textContent ?? ""));
		expect(denyPressed.length).toBeGreaterThan(0);
	});

	it("no longer authors hooks — points to the Hooks surface instead", async () => {
		wireDefaults({
			show: {
				...EMPTY,
				hooks: [
					{ event: "PreToolUse", matcher: "Bash", command: "echo hook" },
				],
			},
			capabilities: CAPS,
		});
		renderEditor({ scope: { kind: "global" }, projectCount: 0 });
		await screen.findAllByText("ALLOW");
		// No HOOKS filter chip, no hook row editors, no add-hook control.
		expect(
			screen.queryByRole("button", { name: /^HOOKS/ }),
		).toBeNull();
		expect(screen.queryByLabelText("Event")).toBeNull();
		expect(screen.queryByLabelText("Command")).toBeNull();
		// A read-only pointer to /hooks is present instead.
		const link = screen.getByRole("link", { name: "Hooks" });
		expect(link).toHaveAttribute("href", "/hooks");
	});

	it("omits the hooks key from the permissions_set save payload", async () => {
			...
			async (cmd: string, args?: unknown): Promise<unknown> => {
				switch (cmd) {
					...
					case "permissions_doctor":
						return { findings: [], danger_count: 0 };
					case "permissions_set":
						lastPayload = (args as { payload?: Record<string, unknown> })
							.payload;
						return {
							changed: true,
							normalized: {
								...EMPTY,
								allow: [{ pattern: "Bash(npm:test)", kind: "allow" }],
							},
						};
					default:
						return undefined;
		...
		expect(lastPayload && "hooks" in lastPayload).toBe(false);
		expect(lastPayload).toHaveProperty("allow");
	});
