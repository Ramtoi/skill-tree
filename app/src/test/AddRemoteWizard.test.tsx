...
// ─── R1/R2/R10 — credential-step dead-end fixes ───────────────────────────────

describe("AddRemoteWizard — credential step recovery", () => {
	async function walkToCredentials(handler: (cmd: string) => unknown) {
		vi.mocked(invoke).mockImplementation((async (cmd: string) =>
			handler(cmd)) as never);
		renderWithProviders(
			<AddRemoteWizard onClose={() => {}} onCreated={() => {}} />,
		);
		await userEvent.click(screen.getByText("Hermes"));
		...
		await screen.findByText((_c, el) =>
			el?.classList.contains("remote-fpr") ?? false,
		);
		await userEvent.click(screen.getByRole("checkbox"));
		await userEvent.click(screen.getByRole("button", { name: /^Next$/i }));
	}

	it("surfaces a failed ssh-copy-id as a PERSISTENT inline panel with a copyable fallback", async () => {
		await walkToCredentials((cmd) => {
			if (cmd === "remote_setup_key")
				return {
					success: false,
					output:
						"ssh-copy-id to hermes@moon-base failed: Permission denied (publickey).",
				};
			return baseHandler(cmd);
		});
		await userEvent.click(
		...
		// Persistent panel with the backend message + a copyable terminal fallback.
		expect(
			await screen.findByText(/Couldn't install the key/i),
		).toBeInTheDocument();
		expect(screen.getByText(/Permission denied/i)).toBeInTheDocument();
		const fallback = screen.getAllByText("ssh-copy-id hermes@moon-base");
		expect(fallback.length).toBeGreaterThanOrEqual(1);
		expect(
			screen.getAllByRole("button", { name: /^Copy$/i }).length,
		).toBeGreaterThanOrEqual(1);
		// Persistent: still present after a re-render tick (not a vanishing toast).
		expect(
			screen.getByText(/Couldn't install the key/i),
		).toBeInTheDocument();
	});

	it("renders the exact ssh-keygen command when no SSH key exists", async () => {
		await walkToCredentials((cmd) => {
			if (cmd === "remote_setup_key")
				return {
					success: false,
					output:
						"No SSH public key found (looked for ~/.ssh/id_ed25519.pub). Generate one first, then retry:\n\n    ssh-keygen -t ed25519\n",
				};
			return baseHandler(cmd);
		});
		await userEvent.click(
		...
		expect(
			await screen.findByText("ssh-keygen -t ed25519"),
		).toBeInTheDocument();
	});

	it("gates Next on a passed Test connection (no self-attest needed)", async () => {
		await walkToCredentials((cmd) => {
			if (cmd === "remote_probe")
				return {
		...
		// Next is disabled until we either test OR self-attest.
		expect(screen.getByRole("button", { name: /^Next$/i })).toBeDisabled();
		await userEvent.click(
			screen.getByRole("button", { name: /Test connection/i }),
		);
		await waitFor(() =>
			expect(invoke).toHaveBeenCalledWith("remote_probe", {
				sshHost: "hermes@moon-base",
				hostKey: "SHA256:TESTfingerprintTESTfingerprintTESTfinger00",
			}),
		);
		expect(
			await screen.findByText(/The box accepts your key/i),
		).toBeInTheDocument();
		expect(screen.getByRole("button", { name: /^Next$/i })).toBeEnabled();
	});

	it("a failed Test connection shows a plain-language hint and keeps Next gated", async () => {
		await walkToCredentials((cmd) => {
			if (cmd === "remote_probe")
				return {
					ssh_host: "hermes@moon-base",
		...
		expect(
			await screen.findByText(/refused our key/i),
		).toBeInTheDocument();
		expect(screen.getByRole("button", { name: /^Next$/i })).toBeDisabled();
	});
});
