...
/**
 * The wizard now opens on a fresh-vs-restore decision (backup-and-restore §9),
 * so every test in this file — all of which exercise the IMPORT branch — first
 * advances past that choice. `fireEvent` is act-wrapped and synchronous, so the
 * existing synchronous test bodies are otherwise untouched.
 *
 * The choice screen itself, and the restore branch, are covered separately in
 * `BootstrapRestoreFirst.test.tsx`.
 */
function renderImportStage(state: BootstrapState) {
  const result = renderWithProviders(<BootstrapWizard state={state} />);
  fireEvent.click(screen.getByTestId("choose-fresh"));
  return result;
}
