...
const SCHEMA: RiskSchemaEntry[] = [
	...
	},
	{
		...
		severity: "warning",
		explanation:
			"An allow and a deny share the same pattern. Deny wins, so the allow is dead.",
	},
];
...
describe("detectRisks", () => {
	...
	it("flags CONTRADICTORY_RULE when an allow + deny share a pattern (dead allow)", () => {
		const perms: NormalizedPermissions = {
			...emptyPerms(),
			allow: [{ pattern: "Bash(git push:*)", kind: "allow" }],
			deny: [{ pattern: "Bash(git push:*)", kind: "deny" }],
		};
		const findings = detectRisks(perms, SCHEMA);
		const contradiction = findings.find(
			(f) => f.code === "CONTRADICTORY_RULE",
		);
		expect(contradiction).toBeTruthy();
		expect(contradiction?.detail).toBe("Bash(git push:*)");
	});
