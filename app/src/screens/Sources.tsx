...
const STATUS_META: Record<string, { color: string; label: string }> = {
	...
	"update-available": { color: "var(--amber)", label: "update available" },
			...
			<div className="source-card-head">
				...
				<div className="source-card-id">
					<div className="source-card-name">
						...
						{updateAvail && <Tag color="var(--amber)">UPDATE AVAILABLE</Tag>}
			...
			{isExternal && !disabled && (
				<div className="source-card-detail">
					<div className="source-detail-row">
						...
						{updateAvail && (
							<>
								<Icon name="arrowRight" size={11} style={{ color: "var(--amber)" }} />
								<span className="v text-mono" style={{ color: "var(--amber)" }}>
...
function AddSourceModal({ onClose }: AddSourceModalProps) {
	...
	async function onApply() {
		setBusy(true);
		try {
			const args = ["source", "add", "git", url, "--json"];
			if (id) args.push("--id", id);
			if (branch) args.push("--branch", branch);
			if (path) args.push("--path", path);
			const res = await hubCmd(args);
			if (!res.success) throw new Error(res.output);
			const payload = JSON.parse(res.output);
			if (!payload.ok) throw new Error(payload.error || "add failed");
			await queryClient.invalidateQueries({ queryKey: ["registry"] });
			toast.success(`Added source with ${payload.registered.length} skill(s)`);
			onClose();
		} catch (err) {
				...
				{preview && (
					<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
						...
						<div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
							...
							<Button variant="primary" onClick={() => void onApply()} disabled={busy || preview.counts.new === 0}>
								Apply · register {preview.counts.new} new
							</Button>
						</div>
					</div>
				)}
