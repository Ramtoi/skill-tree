...
export function Sources() {
	...
	async function confirmRemoveSource() {
		...
		try {
			...
			toast.error("Couldn't remove source", String(err));
		} finally {
			setBusy(false);
		}
	}
...
function AddSourceModal({ onClose, registry }: AddSourceModalProps) {
	const toast = useToast();
	...
	// The id we'd register with: the URL-derived slug, bumped past any collision
	// so the pre-filled default always applies cleanly.
	const suggestedId = useMemo(
		() => suggestFreeSourceId(deriveSourceIdFromUrl(url), taken),
		[url, taken],
	);
	...
	const canApply =
		...
		(preview.counts.new > 0 ||
			conflicts.some((c) => decisionFor(c.name) !== "skip"));
					...
					<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
						<div className="source-add-summary">
							...
							{path && (
								<div>
									<span>Path</span>
									<strong className="text-mono">{path}</strong>
								</div>
							)}
						</div>
