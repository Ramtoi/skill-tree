...
const TOKEN_BYTE_ENCODER =
	typeof TextEncoder !== "undefined" ? new TextEncoder() : null;
function byteLength(text: string): number {
	if (!text) return 0;
	return TOKEN_BYTE_ENCODER
		? TOKEN_BYTE_ENCODER.encode(text).length
		: text.length;
}
...
export function SkillEditor() {
	...
	const descApprox = useMemo(
		() => estimateTokensFromBytes(byteLength(description)),
		[description],
	);
	const bodyApprox = useMemo(
		() => estimateTokensFromBytes(byteLength(content)),
		[content],
	);
	const totalApprox = useMemo(
		() =>
			estimateTokensFromBytes(
				byteLength(composeDocForTokens(name, description, content)),
			),
	...
	const tokenKey = `${name} ${description} ${content}`;
	useEffect(() => {
	...
	const descTokens = exactCurrent ? exactTokens!.desc : descApprox;
	const bodyTokens = exactCurrent ? exactTokens!.body : bodyApprox;
	const totalTokens = exactCurrent ? exactTokens!.total : totalApprox;
