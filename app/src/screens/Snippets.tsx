...
type EditMode = "edit" | "preview" | "diff";

function normalizeBody(s: string): string {
	return s.replace(/\r\n/g, "\n").replace(/\s+$/g, "");
}

// ─── Segmented Edit / Preview / Diff control ────────────────────────────────
function EditModeChips({
	value,
	onChange,
}: {
	value: EditMode;
	...
	const views: { id: EditMode; label: string; icon: string }[] = [
...
function AppliedLocationsPanel({
	...
	async function updateEverywhere() {
		try {
			...
			toast.error("Update everywhere failed", String(err));
		}
	}
		...
		<div className="side-panel-block snip-applied">
			<h4>
				...
				{outdatedCount > 0 && (
					...
					>
						<Icon name="state.update" size={11} /> Update everywhere ·{" "}
						{outdatedCount}
					</button>
				)}
			</h4>
...
function SnippetDetail({
	...
	return (
		<div className="editor-grid snip-detail-grid">
			<div className="editor-main">
				<div className="snip-detail-head">
					...
					<div className="snip-detail-actions">
						<EditModeChips value={mode} onChange={setMode} />
						<Button
							variant="primary"
							icon="save"
							kbd="⌘S"
				...
				<div className="snip-code">
					{mode === "edit" && <CodeAreaEdit content={body} onChange={setBody} />}
					{mode === "preview" && <CodeAreaPreview content={body} />}
					{mode === "diff" && (
						<CodeAreaDiff original={snippet.body ?? ""} current={body} />
					)}
				</div>
...
export function Snippets() {
	...
	return (
		<>
			...
			<div className="main-body snip-screen">
				{/* ── Library list ── */}
