...
function RemoteCard({
	...
	const healthChip = ((): { tone: string; label: string; detail?: string } => {
		...
		if (healthError)
			return {
				tone: "neutral",
				label: "check failed",
				detail: String(healthErrorDetail),
			};
		if (healthLoading || health === undefined)
			return { tone: "neutral", label: "checking…" };
		if (health.reachable === false) return { tone: "neutral", label: "unreachable" };
		if (health.authenticated === false || health.ok === false)
			return { tone: "error", label: "auth failed" };
		return { tone: "ok", label: "reachable", detail: health.detail };
	})();
			...
			<div className="remote-card-meta">
				<div className="remote-meta-row">
					...
					<span
						...
						title={healthChip.detail || undefined}
					>
						<span className="dot" />
						{healthChip.label}
					</span>
