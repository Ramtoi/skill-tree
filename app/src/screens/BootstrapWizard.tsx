...
/** Shared full-bleed frame for the non-import stages (the import stage keeps
 *  its own scroll-body + sticky-footer layout). */
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: 20,
        borderRadius: 10,
        border: `1px solid ${primary ? "var(--violet)" : "var(--bg-3)"}`,
        background: "var(--bg-1)",
        cursor: "pointer",
        color: "inherit",
        font: "inherit",
    ...
    >
      ...
      <div style={{ fontSize: 15, fontWeight: 600, color: "var(--fg-strong)" }}>{title}</div>
      <div style={{ fontSize: 12.5, color: "var(--fg-mid)", lineHeight: 1.55 }}>{body}</div>
      <div style={{ fontSize: 12, color: primary ? "var(--violet)" : "var(--fg-mute)", marginTop: 4 }}>
        {cta} →
      </div>
    </button>
  );
    ...
    return (
      <WizardFrame>
        <div data-testid="bootstrap-choose">
          <h1 style={{ fontSize: 22, margin: 0, color: "var(--fg-strong)" }}>
            Set up Skill Tree
          </h1>
          <p style={{ marginTop: 8, color: "var(--fg-mid)", lineHeight: 1.5 }}>
            Starting fresh, or bringing a hub over from another machine?
          </p>
          <div
            style={{
              marginTop: 28,
          ...
          >
            <ChoiceCard
              ...
              icon="spark"
              title="Set up fresh"
              body="Build a new hub here. Skill Tree will offer to import skills it finds in ~/.claude, ~/.codex, and ~/.pi."
              cta="Start setup"
              testId="choose-fresh"
            ...
            <ChoiceCard
              ...
              body="Pull an existing hub from a snapshot repo. Skips the import step — the snapshot already has everything."
