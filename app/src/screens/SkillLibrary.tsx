...
function LibraryView({
	...
	async function adoptCandidate(cand: ProjectSkillCandidate) {
		...
		if (!res.success) {
			toast.error("Couldn't import skill", res.output.trim() || undefined);
	...
	}, [allSkills, q, kindFilter, invocationFilter, bundleFilter, sourceFilter, registry.bundles, sourceBySkill]);
		...
		<>
			...
			>
				<div className="library-filter-popover">
					...
					<div className="filter-group">
						<span className="filter-label">TRIGGERING</span>
						<Chips>
							{(
								[
									["all", "All"],
								...
								<Chip
									...
									onClick={() =>
										setInvocationFilter(
											invocationFilter === value ? "all" : value,
										)
									}
								>
									{label}
