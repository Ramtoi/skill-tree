...
export function ProjectWorkspace() {
	...
	useEffect(
		() => () => {
			for (const t of equipFadeTimers.current) window.clearTimeout(t);
		},
		[],
	);
	...
	// B4a-07 / B4b-04: the project tab strip scrolls horizontally at narrow
	// widths (`.main-subheader-left` overflow-x). Keep the active tab in view so
	// a later tab (e.g. Sub-Agents) isn't left clipped off the right edge.
	useEffect(() => {
		const active = document.querySelector<HTMLElement>(
			'.main-subheader-left [role="tab"][aria-pressed="true"]',
		);
	...
	const orderedEquipped = useMemo(() => {
		...
		return names.sort((a, b) => {
			...
			const aDirectIndex = projGuard?.enabled.indexOf(a) ?? -1;
			const bDirectIndex = projGuard?.enabled.indexOf(b) ?? -1;
	...
	}, [equipped, equipSort, projGuard?.enabled, recentlyEquipped]);
	...
	const bundleProvidedSet =
		projGuard && registry
			...
			: new Set<string>();
	const directCount =
		projGuard && registry ? directOnlySkills(projGuard, registry).length : 0;
	...
	if (!projectName || !registry?.projects?.[projectName]) {
		return (
			<EmptyState
				...
				description="Register a project to equip skills and apply bundles to it."
				action={
					<Button
						variant="primary"
						icon="plus"
	...
	async function createEnvFile() {
		try {
			...
			toast.error("Couldn't create .env", String(err));
		}
	}
	...
	async function adoptCandidate(cand: ProjectSkillCandidate) {
		try {
			...
			toast.error("Couldn't import skill", String(err));
			throw err;
		}
	}
	...
	async function enableSkill(skillName: string) {
		...
		try {
			...
			toast.error("Couldn't equip skill", String(err));
		}
	}
	...
	async function disableSkill(skillName: string) {
		...
		try {
			...
			toast.error("Couldn't unequip skill", String(err));
		}
	}
	...
	async function applyBundle(bundleName: string) {
		try {
			...
			toast.error("Couldn't apply bundle", String(err));
		}
	}
	...
	async function removeBundle(bundleName: string) {
		try {
			...
			toast.error("Couldn't remove bundle", String(err));
		}
	}
	...
	async function setInvocationOverride(
		...
		try {
			...
			toast.error("Couldn't set triggering override", String(err));
		}
	}
				...
				<ResizableSplit
					...
					<div className="workspace-main">
							...
							<StatCard
								...
								sub={
									globalBundles.length > 0
										? `+${globalBundles.length} global auto-applied`
						...
						<div className="loadout-section">
							...
							{/* Project hooks — quick attach/detach toggles (authoring on /hooks) */}
							<ProjectHooksCard projectName={projectName} />
						</section>
						...
						{affinitySkips.length > 0 && (
							<div className="affinity-skip-banner" role="status">
								...
								<span>
									...
									won't reach any harness — no installed harness matches their{" "}
