...
export interface RemoteHealthResult {
	...
	/** Connector home/install present (Hermes set up). False + authenticated ⇒
	 *  "not set up". Absent on legacy payloads → treat as ready. */
	ready?: boolean;
	/** Machine tag (unified backend vocabulary). Absent on legacy payloads. */
...
/** One remote-doctor finding (`hub remote doctor --json`). Mirrors the CLI shape:
 *  a per-remote risk with a machine `code`, a `severity`, and a human `detail`. */
export interface RemoteDoctorFinding {
	remote: string;
	code: string;
...
/** Aggregate remote risk rollup (`hub remote doctor --json`): host-key mismatch
 *  (danger/MITM), unreachable sync_enabled remotes, stale sidecars, unresolved
 *  drift. NON-BLOCKING: the query runs in the background so the list renders
 *  instantly; the danger banner appears only when the rollup resolves. Kept OUT
 *  of the `["remotes"]` invalidation prefix (own key) so an equip click doesn't
 *  re-trigger the expensive per-remote health+plan sweep; refetch it explicitly
 *  (a Re-check button) instead. */
export function useRemoteDoctor(enabled = true) {
	return useQuery({
		queryKey: ["remote-doctor"],
		queryFn: () => invoke<RemoteDoctorReport>("remote_doctor"),
		enabled,
		retry: false,
		staleTime: 60_000,
