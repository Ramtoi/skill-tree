 ...
 * visible duration — there is no undo store and no multi-level stack. Because
 * this is the *only* undo path, the toast gets an explicit longer duration
 * (7s vs. the 3.2s success default) and pauses on hover (B3-13).
 *
 ...
 */
/** Auto-dismiss window for an undoable success toast — the full undo lifetime.
 *  Longer than the 3.2s success default because this is the sole undo path. */
      ...
      const runUndo = () => {
        void (async () => {
          try {
            ...
            toast.error("Couldn't undo", String(err));
          }
        })();
      };
