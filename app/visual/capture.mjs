...
const ONLY = (process.env.ST_VISUAL_ONLY ?? "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
const HEIGHT = 900;
...
const SCENES = [
  ...
  },
  {
    ...
    waitFor: '[data-testid="agent-docs-show-all-markdown"]',
  },
  {
    ...
    waitFor: '[role="tab"]',
    prep: async (page) => {
      // On narrow widths the map panel auto-collapses behind a reopen tab; open
      // it first so the toggle is interactable, then flip "Show all Markdown
      // files" so the tree lists every .md.
  ...
  // ─── Local Agent Usage (ccusage) ───────────────────────────────────────────
  {
    id: "usage-empty",
    label: "Local Agent Usage — first-run empty",
    path: "/?usageEmpty=1#/usage",
    waitFor: "text=No cached usage scan yet",
  },
  {
    id: "usage-success",
    label: "Local Agent Usage — cached dashboard",
    path: "/#/usage",
    waitFor: "text=Largest sessions",
  },
  {
    id: "usage-no-usage",
    label: "Local Agent Usage — no local usage found",
    path: "/?usageNoUsage=1#/usage",
    waitFor: "text=No local harness usage was detected yet",
  },
  {
    id: "usage-access-error",
    label: "Local Agent Usage — permission/local access issue",
    path: "/?usageAccessError=1#/usage",
    waitFor: "text=Skill Tree cannot access local usage logs",
  },
  {
    id: "usage-ccusage-failure",
    label: "Local Agent Usage — ccusage failure diagnostics",
  ...
  },
  {
    ...
    label: "Hooks — Editor (built-in lsp-report · read-only cmd + per-language table)",
...

  {
    ...
    label: "Backup — configured, in sync (repo + auth ladder + danger zone)",
    path: "/#/backup",
    waitFor: '[data-testid="backup-screen"]',
  },
  {
    id: "backup-unconfigured",
    label: "Backup — never set up (empty state)",
    path: "/?backupUnconfigured=1#/backup",
    waitFor: '[data-testid="backup-screen"]',
  },
  {
    id: "backup-stale",
  ...
  },
  {
    ...
    waitFor: '[data-testid="pat-unavailable"]',
  },
  {
    id: "backup-pat-form",
    label: "Backup — masked PAT entry form",
    path: "/#/backup",
    waitFor: '[data-testid="open-pat-form"]',
    prep: async (page) => {
  ...
  },
  {
    ...
    path: "/#/backup",
    waitFor: '[data-testid="restore-danger-zone"]',
    prep: async (page) => {
      await page.fill("#restore-source", "git@github.com:me/skill-hub-backup.git").catch(() => {});
