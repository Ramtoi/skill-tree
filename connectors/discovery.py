...
hardcoded imports. That made importing the package non-trivial (it pulled
Hermes) and left distribution un-pluggable.
...
key):
    ...
    1. builtin       — `connectors.hermes.HermesConnector` (in `code_home()`,
                       therefore public by definition: a builtin connector MUST
                       set `publishable = True`)
    2. entry-point   — distributions exposing `skill_hub.connectors` entry points
    3. drop-in       — `*.py` files and `*/__init__.py` packages under
                       `data_home()/connectors/`

**Location is the shipping boundary.** Anything not in `code_home()` cannot
reach a built artifact or the public source mirror; private/local plugins live
as drop-ins in the data home. The old in-repo "private" phase (a private plugin
package inside the code tree) is gone — its bytes leaked into the built `.app`
through `bundle.resources`, because a path inside the tree has to be
*remembered* by every consumer of that tree.
...
catalog can report builtin / entry-point / drop-in.
"""
...
#: registry key → discovery source ("builtin" | "entry-point" | "drop-in").
_SOURCE: dict[str, str] = {}

#: Entry-point group a distribution advertises a connector under.
ENTRY_POINT_GROUP = "skill_hub.connectors"

#: Module-name namespace a drop-in (and its submodules) is imported under.
_DROPIN_PREFIX = "skill_hub_dropin_"
...
def _reset_for_tests() -> None:
    ...
    the source map, and the memo flag, and evicts every loaded drop-in module so
    a re-discovery re-executes it (a drop-in's registrations only happen at
    import time).
    """
    global _discovered, _in_progress
    _discovered = False
    ...
    for name in [n for n in sys.modules if n.startswith(_DROPIN_PREFIX)]:
...
def _load_dropin(modname: str, path, *, is_package: bool) -> None:
    ...
    before = set(connectors.REMOTE_CONNECTORS)
    mod_name = f"{_DROPIN_PREFIX}{modname}"
    try:
        if is_package:
