...
for name in sys.stdin.buffer.read().split(b"\0"):
    ...
    for root, dirs, files in os.walk(d):
        for f in files:
            if f.endswith(".hub-bak") or ".hub-tmp." in f:
                continue
...
def _is_hub_internal_file(basename: str) -> bool:
    """True for a hub-created backup/temp sibling that is NOT skill content.

    `backup_on_change` writes `<file>.hub-bak` and `atomic_write` stages
    `<file>.hub-tmp.<pid>.<n>` siblings — both land INSIDE the managed skill dir.
    They must be excluded from every content hash (`dir_sha256` / the batch
    digest) and from a pulled skill tree, or a single fast-forward would leave a
    `.hub-bak` behind and every subsequent `plan()` would mis-classify the skill
    as REMOTE_DRIFTED (phantom drift that never clears). Mirrored byte-for-byte in
    `_DIR_SHA_BATCH_PY`.
    """
    return basename.endswith(".hub-bak") or ".hub-tmp." in basename
...
class ProbeClassification:
    """Uniform classification of an ssh probe/health failure.

    `detail_kind` uses the shared remote vocabulary
    (`ready`/`unreachable`/`auth_failed`/`host_key_mismatch`); the boolean flags
    map onto `HealthResult`'s fields so both `health_check()` and the wizard's
    pre-registration probe classify a transport exception identically.
    """
...
def classify_probe_exception(exc: BaseException) -> "ProbeClassification":
    """Map an ssh verify/authenticate exception to a `ProbeClassification`.

    Single source of truth shared by `HermesConnector.health_check`,
    `CodexWorkersConnector.health_check`, and `cmd_remote_probe` so a transient
    keyscan blip is never mislabelled a MITM:

      * a `HostKeyMismatch` whose message is the "could not read host key"
        unreadable case → ``unreachable`` (benign — ssh-keyscan couldn't reach
        the box), NOT a fingerprint mismatch;
      * a genuine `HostKeyMismatch` → ``host_key_mismatch`` (reachable);
      * a `SshCommandError` carrying an auth string (permission denied /
        publickey / authentication) → ``auth_failed`` (reachable, host-key OK);
      * any other `SshCommandError` / exception → ``unreachable``.
    """
    if isinstance(exc, HostKeyMismatch):
        low = str(exc).lower()
        if "could not read" in low or "to verify" in low:
            return ProbeClassification("unreachable", False, False, False, str(exc))
        return ProbeClassification("host_key_mismatch", True, False, False, str(exc))
    if isinstance(exc, SshCommandError):
        ...
        if (
            ...
            return ProbeClassification(
                "auth_failed", True, False, True, msg or "authentication failed"
            )
        return ProbeClassification("unreachable", False, False, False, msg)
    return ProbeClassification("unreachable", False, False, False, str(exc))
...
class SshTransport:
    ...
    def authenticate(self) -> None:
        """Prove reachability + authentication with a trivial remote command.

        Runs `true` on the box — a command that exists everywhere and touches
        nothing — so a health check can tell "the box accepts our key" apart from
        "the connector's home dir is installed" (a SEPARATE `probe`). Raises
        `SshCommandError` on ANY ssh-level failure (exit 255: connection refused /
        DNS / timeout / `Permission denied (publickey)`); the caller inspects the
        stderr to split unreachable from an auth failure.
        """
        self.verify_host_key()
        res = self._run(self._ssh_argv("true"))
        if res.returncode != 0:
            raise SshCommandError(["ssh", self.host, "true"], res.returncode, res.stderr)

    def probe(self, home: str) -> bool:
        """Cheap home-dir existence probe — `test -d <home>` style.
        ...
        Use `authenticate()` first to separate an auth failure from a merely
        missing home dir — this method alone cannot tell them apart on a 255.
        """
        self.verify_host_key()
    ...
    def list_files(self, path: str) -> list[str]:
        ...
        for ln in res.stdout.splitlines():
            ...
            # Skip hub-internal backup/temp siblings — they are not skill content
            # and would otherwise pollute dir_sha256 (phantom REMOTE_DRIFTED after
            # a fast-forward leaves a `.hub-bak`) and a pulled skill tree.
            if _is_hub_internal_file(rel.rsplit("/", 1)[-1]):
                continue
            out.append(rel)
        return out
