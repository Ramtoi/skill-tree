...
class ApplyResult:
    ...
    skipped: tuple[str, ...] = ()
    errors: tuple[str, ...] = ()
    #: Names force-written via `apply(force_names=…)` — an explicit user override
    #: (`hub remote resolve --op push`) that clobbered a drifted/conflict artifact.
    forced: tuple[str, ...] = ()

    @property
    def changed(self) -> bool:
        return bool(self.created or self.fast_forwarded or self.removed or self.forced)
...
class HealthResult:
    ...
    `ready` distinguishes "connected + authenticated, but the connector's remote
    home / install is not present yet" (e.g. `~/.hermes` missing on a box that
    otherwise accepts our key) from a genuine auth/reachability failure. It
    defaults to True so existing connectors that only set reachable/auth/host-key
    keep their previous `ok` semantics.

    `detail_kind` is a stable machine tag for the state so callers (dispatch, the
    CLI, the app chips/banners) can classify WITHOUT string-matching `detail`:
      "" (unset/legacy) | "ready" | "unreachable" | "auth_failed"
      | "host_key_mismatch" | "home_missing"
    """
    ...
    detail: str = ""
    #: See class docstring — home/install present. Default True for back-compat.
    ready: bool = True
    #: Machine-readable state tag (see class docstring). Default "" (legacy).
    detail_kind: str = ""
    ...
    def ok(self) -> bool:
        return (
            ...
            and self.host_key_match
            and self.ready
        )


# ─────────────────────────────────────────────────────────────────────────────
# Box-side helper install plan (the `setup_helper_plan` hook's return type)
# ─────────────────────────────────────────────────────────────────────────────


@dataclass
class HelperPlan:
    """A connector's box-side helper install plan — PRINTED, never executed.

    Returned by `RemoteConnector.setup_helper_plan()` and rendered by
    `hub remote setup-helper`. Every field is plain data so the CLI can print it
    (text or `--json`) with **zero** knowledge of the connector: which artifact
    to copy, where it lands, and the exact commands the user runs on the box.

    - `helper_src`  — absolute local path of the artifact to copy to the box.
      A connector resolves it from its OWN location (`Path(__file__).parent / …`)
      so the plan is correct from any install location (repo, drop-in, pip).
    - `install_path` — absolute destination path on the box.
    - `box_commands` — the exact, copy-pasteable commands (never run by hub).
    - `public_key` / `authorized_keys_line` — the dedicated key this plan pins,
      when the connector uses a forced-command key (None otherwise).
    - `notes` — free-form advisory lines shown under the commands.
    """

    helper_src: str
    install_path: str
    box_commands: list = field(default_factory=list)
    public_key: Optional[str] = None
    authorized_keys_line: Optional[str] = None
    title: str = "Box helper — install plan (NOT executed)"
...
class RemoteConnector(ABC):
    ...
    # --- box-helper install hook (de-special-cases core) --------------------

    def setup_helper_plan(self, target=None) -> Optional[HelperPlan]:
        """Resolve this connector's box-side helper install plan, or None.

        Returns `None` by default → `hub remote setup-helper` reports that the
        connector has no box-side helper. A connector that requires a privileged
        helper (e.g. a forced-command script pinned in root's `authorized_keys`)
        overrides this to return a `HelperPlan` whose `helper_src` is resolved
        from the connector's OWN package location (`Path(__file__).parent / …`),
        so it stays correct wherever the plugin is installed.

        `target` is the resolved `RemoteTarget` when the user named a remote id,
        else `None` (pre-registration) — the connector must tolerate both. The
        hook may generate/ensure its own key material (idempotent); raising
        `SystemExit` (via `hub.fail`) or any exception surfaces as a CLI error.
        """
        return None
    ...
    def apply(
        ...
        *,
        ...
        `force_names` is the explicit-user-override escape hatch used by
        `hub remote resolve --op push`: for a name in this set the connector
        writes the local payload REGARDLESS of drift status (`SKIP_REMOTE_DRIFTED`
        / `SKIP_CONFLICT` are treated as writes, the TOCTOU drift-abort is
        bypassed) and re-bases the ownership sidecar so the artifact reads
        in-sync afterwards. Forcing is STRICTLY SCOPED: only actions whose
        `.name` is in `force_names` are force-written; every other artifact obeys
        the normal `allow`-gated path. When `force_names` is empty, behavior is
        identical to the plain plan-then-apply sync path (no artifact is ever
        clobbered).
        """
