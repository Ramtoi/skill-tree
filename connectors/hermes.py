...
class HermesConnector(RemoteConnector):
    ...
    def health_check(self, target) -> HealthResult:
        """Reachable + auth + host-key pin match + Hermes-installed (`~/.hermes`).

        The probe is SPLIT so a fresh box is never mis-reported as an auth
        failure (R3/R4): first prove auth with a trivial command, THEN check the
        home dir. The four distinguishable outcomes carry a `detail_kind` tag:
          * host_key_mismatch — pinned fingerprint ≠ live (possible MITM; RED)
          * unreachable       — no connection / DNS / timeout (neutral)
          * auth_failed       — reachable but the box rejects our key (RED)
          * home_missing       — authenticated, but `~/.hermes` is absent — Hermes
                                is not installed on this box (NEUTRAL, informative)
          * ready             — fully good to sync
        """
        transport = self._transport(target)
        # Step 1: verify the pinned host key + prove reachability + auth with a
        # trivial command. `authenticate()` verifies the host key internally, so a
        # single call covers both a host-key mismatch and an auth/connection
        # failure. The shared classifier distinguishes a real fingerprint mismatch
        # (host_key_mismatch) from a keyscan blip ("could not read host key" →
        # unreachable) and an auth rejection from a merely-unreachable box.
        try:
            transport.authenticate()
        except Exception as e:  # HostKeyMismatch / SshCommandError / stall
            cls = classify_probe_exception(e)
            return HealthResult(
                reachable=cls.reachable,
                authenticated=cls.authenticated,
                host_key_match=cls.host_key_match,
                ready=False,
                detail=cls.detail,
                detail_kind=cls.detail_kind,
            )
        # Step 2: authenticated — now check whether Hermes is installed (its home
        # dir exists). A missing home is NOT an auth failure; it is a neutral
        # "connected, but Hermes isn't set up on this box yet".
        try:
            home = self._resolve_home(target, transport)
            home_present = transport.probe(home)
        except Exception as e:  # a mid-op stall between auth and probe
            return HealthResult(
                reachable=False, ready=False, detail=str(e),
    ...
    def apply(
        ...
        plan: RemotePlan,
        *,
        allow: frozenset = DEFAULT_ALLOW,
        force_names: frozenset = frozenset(),
    ) -> ApplyResult:
        ...
        `force_names` is the explicit-user-override escape hatch
        (`hub remote resolve --op push`): for an action whose `.name` is in this
        set the local payload is written REGARDLESS of drift — `SKIP_REMOTE_DRIFTED`
        / `SKIP_CONFLICT` / `CREATE` / `FAST_FORWARD` / (MISSING→CREATE) all become
        a forced write and the TOCTOU drift-abort is bypassed — then the sidecar is
        re-based so the artifact reads in-sync. Forcing is STRICTLY SCOPED: an
        action NOT in `force_names` obeys the normal `allow`-gated path unchanged,
        so a co-drifted sibling is never touched. When `force_names` is empty this
        method behaves identically to the plain sync path.
        """
        transport = self._transport(target)
        home = self._resolve_home(target, transport)
        paths = _Paths(home)
        ...
        # Forced write-actions (explicit user override) — a forced push always
        # writes local content, so REMOVE/NOOP are not forced (nothing to push).
        # A MISSING drift status already plans as CREATE, so CREATE covers it.
        _FORCE_WRITEABLE = frozenset({
            Action.CREATE,
            Action.FAST_FORWARD,
        ...
        def _is_forced(a) -> bool:
            return bool(force_names) and a.name in force_names and a.action in _FORCE_WRITEABLE

        # Ensure the external_dirs registration once if any skill is written
        # (normal allow-gated writes OR a forced skill push).
        will_register_external = any(
            a.kind == "skill"
            and (
                (a.action in allow and a.action in (Action.CREATE, Action.FAST_FORWARD))
                or _is_forced(a)
            )
        ...
        for action in plan.actions:
            if _is_forced(action):
                # Explicit user override: write local content regardless of drift.
                try:
                    wrote = self._apply_write(
                        target, transport, paths, action, desired_lookup, force=True
                    )
                    if wrote:
                        forced.append(action.name)
                    else:  # pragma: no cover - forced write only returns False on byte-identical
        ...
        if created or fast_forwarded or forced:
            try:
    ...
    def _apply_write(self, target, transport, paths, action: PlannedAction, desired_lookup, *, force: bool = False) -> bool:
        ...
        `force=True` (explicit `hub remote resolve --op push` override) BYPASSES
        the F4 TOCTOU drift-abort: the local payload is written even when the
        artifact is REMOTE_DRIFTED / CONFLICT. The sidecar is still re-based on a
        successful write (that is what clears the drift). A genuinely byte-identical
        payload short-circuits inside the per-kind writer and returns True without
        an SSH write, so the sidecar still rebases.
        """
        ...
        #
        # `force` skips this guard entirely: an explicit user push is a deliberate
        # decision to clobber the box with local content.
        sc = _sidecar.read_sidecar(target.id, surface)
