...
macOS is the primary target; Linux is best-effort, Windows untested. Releases do
attach a best-effort, unsigned Windows installer built by CI
(`SkillTree-windows-x64-setup.exe` / `.msi`) — it is unsupported, ships no
bundled Python runtime, and has no auto-update. You need:
