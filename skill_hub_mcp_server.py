...
sync; permissions/harness are READ-ONLY. There are intentionally NO
...
def _call_cmd(fn, ns):
    """Run a hub cmd_* with stdout/stderr captured and SystemExit trapped.

    Returns (ok: bool, output: str, error: str|None). `ok` is False when the
    command exited non-zero (hub.fail / sys.exit) or raised.
    """
    out = io.StringIO()
    err = io.StringIO()
    error = None
