...
Third-party connectors need no fork. Discovery runs three sources in order —
**builtin** (Hermes) → **entry points** (group `skill_hub.connectors`, for
pip-installed distributions) → the **drop-in directory** `data_home()/connectors/`
(each `*.py` file or package dir; suited to the packaged app, which has no pip
environment).
...
**Publishable vs private — location is the boundary.** A connector's *location*
decides whether it can ever ship; the `publishable` flag declares the intent and
is now enforced against that location:

| Location | Ships in the `.app` / mirror? | Source | Rule |
|---|---|---|---|
| `code_home()/connectors/` (in this repo) | yes — a `bundle.resources` entry | `builtin` | **MUST** set `publishable = True` |
| a pip distribution exposing `skill_hub.connectors` | no — the user installs it | `entry-point` | author's choice |
| `data_home()/connectors/` (drop-in) | structurally impossible — neither in git nor in `bundle.resources` | `drop-in` | private/local plugins live here |

One sentence: **if it is not in `code_home()`, it cannot ship; if it is in
`code_home()`, it is public by definition.** There is no third location to
remember to exclude. The invariant is checked twice — a pytest over the in-repo
tree, and `scripts/guard-bundle-privacy.sh` against the built bundle (which also
greps the shipped bytes for private identifiers and refuses any private-looking
path name). The framework (`connectors/base.py`, `drift.py`, `sidecar.py`,
`transport/`, `layouts/`) and `hermes` are the publishable set; the maintainer's
custom control-plane connectors are drop-ins from a private plugin repo, and the
framework carries zero knowledge of them. Layering rule: lower modules never
import higher, and **no connector imports another**.

**Writing a private/local plugin.** One directory per plugin, symlinked (or
copied) into `data_home()/connectors/<name>`:

```bash
mkdir -p ~/.skill-hub/connectors
ln -s ~/Dev/my-plugins/my_connector ~/.skill-hub/connectors/my_connector
hub remote connectors            # appears with SOURCE = drop-in
```

Two rules inside a plugin: **self-imports must be relative** (`from . import x`)
— a drop-in is imported under the alias `skill_hub_dropin_<dir>`, so an absolute
self-import raises `ModuleNotFoundError`, and a *lazy* (in-function) one fails
only at first real use — while **framework imports stay absolute** (`from
connectors.base import ...`, `import hub`). Edits take effect with no app
rebuild, since `hub.py` re-reads the drop-in on every invocation. Removing the
symlink disables the plugin; a remote still referencing it is skipped with a
warning (never a hard sync failure).

**Box-side helper installs.** A connector needing a privileged box-side helper
overrides the optional `setup_helper_plan(target)` hook and returns a
`HelperPlan` (`connectors/base.py`) whose `helper_src` is resolved from
`Path(__file__).parent`, so the plan is correct from any install location.
`hub remote setup-helper <id>` (or `--connector <key>` before the remote is
registered) prints it — hub NEVER executes a box write. Connectors without a
helper inherit the default `None` and the command says so.
`hub remote setup-codex-helper` remains as a deprecated alias for one release.
...
3. **Location.** A publishable connector lives under `connectors/` in this repo
   and MUST set `publishable = True` (a pytest enforces it). A **custom/private**
   connector (`publishable = False`) never enters this tree: it lives in its own
   repo and installs as a drop-in under `data_home()/connectors/<name>` — see
   "Publishable vs private" above.
