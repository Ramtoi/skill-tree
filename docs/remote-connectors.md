...
(`connectors/__init__.py`); `get_connector(key)` resolves one and hard-errors on an
unknown key. Each connector sets two class attributes: `key` (registry key) and
`publishable` (True → may ship in the public mirror).
